...
async function tryTrestle(p) {
  if (!trestleConfigured()) return null;
  try {
    return await enrichPersonViaTrestle(p);
  } catch (_) { return null; }
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // AND not enriched in last 6 hours (or never)
    ...
    for (const p of candidates) {
      ...
      try {
        ...
        // Fire all enrichers in parallel
        const [trestle, pdl, tracerfy, sb, nv] = await Promise.all([
          tryTrestle(p), tryPDL(p), tryTracerfy(p), trySearchBug(p), tryNumVerify(p)
        ]);

        // Merge results (preferring higher confidence per field)
        const merged = {};
        const sourcesUsed = [];
        ...
        let fieldsFilled = 0;
        const updateableFields = ['phone','email','address','zip','employer','occupation','linkedin_url','phone_verified','phone_carrier','phone_line_type'];
