...
// Phase 44A: VoyageAI-powered name reranker. Given the victim's known name and
// a list of candidate names from PDL/Apollo, returns the best-matching index
// and similarity score. Falls back to substring overlap when VoyageAI is
// unavailable (key not set or quota exhausted).
async function rerankCandidatesByName(victimName, candidates, db) {
  if (!Array.isArray(candidates) || candidates.length === 0) return null;
  const candNames = candidates.map(c => {
    if (!c) return '';
    if (typeof c === 'string') return c;
    return c.name || c.full_name || [c.first_name, c.last_name].filter(Boolean).join(' ') || JSON.stringify(c).substring(0, 100);
  });
  if (candidates.length === 1) return { index: 0, score: 1, source: 'single' };
  try {
    const voyage = require('./_voyage_router');
    if (voyage && typeof voyage.rerank === 'function') {
      const ranked = await voyage.rerank(String(victimName || ''), candNames, candidates.length, 'rerank-2.5', db);
      if (Array.isArray(ranked) && ranked.length > 0) {
        const top = ranked[0];
        return { index: top.index, score: top.relevance_score, source: 'voyage_rerank', all: ranked };
      }
    }
  } catch (_) {}
  // Fallback: token overlap
  const vn = String(victimName || '').toLowerCase();
  let best = { index: 0, score: 0, source: 'token_fallback' };
  for (let i = 0; i < candNames.length; i++) {
    const cn = String(candNames[i] || '').toLowerCase();
    const sharedTokens = vn.split(/\s+/).filter(t => t && cn.includes(t)).length;
    if (sharedTokens > best.score) best = { index: i, score: sharedTokens, source: 'token_fallback' };
  }
  return best;
}
  ...
  for (const s of sources) {
    try {
      ...
      while ((m = EMAIL_RE.exec(cleaned)) !== null) {
        ...
        // Phase 39 fix: reject scraper-website own emails (support@thatsthem.com, info@radaris.com, etc.)
        const _at = e.indexOf('@');
        const _domain = _at > 0 ? e.slice(_at + 1) : '';
        if (/(thatsthem|fastpeoplesearch|truepeoplesearch|radaris|whitepages|spokeo|beenverified|peoplefinder|usatoday|cnn|nytimes|washingtonpost|reuters|apnews|bbc|cbsnews|abcnews|nbcnews|foxnews|huffpost)\.com$/i.test(_domain)) continue;
        if (/^(support|info|contact|hello|admin|noreply|no-reply|webmaster|press|editor|abuse|legal|privacy|sales|marketing|hr|jobs|careers|feedback|help|service)@/i.test(e)) continue;
        out.emails.push(e);
...
async function resolveOne(db, personId) {
  ...
  // 2. Apollo (match → optional unlock)
  await refreshPerson();
  if (!isContactComplete(person)) {
    ...
    if (apolloKey) {
      ...
      if (r) {
        ...
        // Phase 44A: if Apollo matched a person but returned no contact, spend
        // a credit and call /people/match with reveal flags.
        const personFound = !!r.data?.person?.id;
        const noContact = ext.phones.length === 0 && ext.emails.length === 0;
        if (personFound && noContact) {
          try {
            const unlock = require('./apollo-unlock');
            const ur = await runSource('apollo_unlock', () => unlock.unlockPerson(db, person));
            if (ur && ur.fields) {
              const u = ur.fields;
              buffers.by_source.apollo_unlock = u;
              if (u.phones && u.phones.length) buffers.phones.push(...u.phones.map(p => ({ p, src: 'apollo_unlock', w: 88 })));
              const ue = [u.email, ...(u.personal_emails || [])].filter(Boolean);
              if (ue.length) buffers.emails.push(...ue.map(e => ({ e, src: 'apollo_unlock', w: 88 })));
            }
          } catch (e) { stats.errors.push(`apollo_unlock:${e.message}`); }
        }
      }
    }
  }
...
module.exports.rerankCandidatesByName = rerankCandidatesByName;
