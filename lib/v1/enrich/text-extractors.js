...
async function extractOnePerson(db, personId) {
  ...
  if (!person.vehicle_vin && vins.length) updates.vehicle_vin = vins[0];
  if (!person.vehicle_plate && plates.length) updates.vehicle_plate = plates[0];
...
async function batchExtract(db, limit = 50) {
  const persons = await db.raw(`
    ...
    WHERE (i.raw_description IS NOT NULL OR i.description IS NOT NULL)
      AND (p.phone IS NULL OR p.address IS NULL OR p.vehicle_vin IS NULL OR p.vehicle_plate IS NULL OR p.insurance_company IS NULL)
