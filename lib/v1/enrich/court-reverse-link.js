...
async function processIncident(db, inc, dsId, dryRun) {
  ...
  for (const c of cases) {
    ...
    if (exists) {
      // Phase 21 Wire #2: existing victim matches plaintiff → set has_attorney=true
      // Don't overwrite if already known.
      const updates = {};
      if (exists.has_attorney !== true) updates.has_attorney = true;
      if (!exists.attorney_firm && c.assigned_to_str) updates.attorney_firm = c.assigned_to_str;
      if (Object.keys(updates).length) {
        updates.updated_at = new Date();
        await db('persons').where('id', exists.id).update(updates).catch(()=>{});
        await db('incidents').where('id', inc.id).update({
          has_attorney_known: true, updated_at: new Date()
        }).catch(()=>{});
        await enqueueCascade(db, {
          person_id: exists.id, incident_id: inc.id,
          trigger_source: 'court_attorney_match',
          trigger_field: 'has_attorney', trigger_value: 'true',
          priority: 7
        }).catch(()=>{});
        added++;
        samples.push({ name: plaintiff, case: caption, attorney_match: true });
      }
      continue;
    }
