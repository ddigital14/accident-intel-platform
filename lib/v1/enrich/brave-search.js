...
async function searchBrave(query, options = {}) {
  ...
  try {
    ...
    const remaining = (resp.headers.get('x-ratelimit-remaining') || '').split(',').map(s => s.trim());
    const quota_remaining_today = parseInt(remaining[0] || '0');
    const quota_remaining_month = parseInt(remaining[1] || '0');
    const quota_exhausted = quota_remaining_month <= 0;
    ...
    return {
      ...
      quota_remaining_today, quota_remaining_month, quota_exhausted
    };
  } catch (e) {
...
async function handler(req, res) {
  ...
  try {
    if (action === 'debug') {
      const key = await getBraveKey(db);
      if (!key) return res.json({ ok: false, error: 'no_brave_key' });
      const url = `${BRAVE_ENDPOINT}?q=pizza&count=3&country=us&safesearch=off`;
      const r = await fetch(url, { headers: { 'Accept': 'application/json', 'X-Subscription-Token': key }, signal: AbortSignal.timeout(8000) });
      const status = r.status;
      const headers = {};
      for (const [k,v] of r.headers.entries()) headers[k] = v;
      const text = await r.text();
      return res.json({ ok: r.ok, status, headers, body_preview: text.slice(0, 2000) });
    }
    if (action === 'health') {
