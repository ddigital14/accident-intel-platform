...
const COUNTY_ENDPOINTS = {
  ...
  'GA:Fulton': {
    url: (addr) => `https://maps.fultoncountyga.gov/arcgis/rest/services/Tax/TaxParcel/MapServer/0/query?where=SITUS_ADDR%20LIKE%20'${encodeURIComponent('%' + addr.toUpperCase() + '%')}'&outFields=*&f=json&resultRecordCount=5`,
    parse: (a) => ({
  ...
  'FL:MiamiDade': {
    url: (addr) => `https://gisweb.miamidade.gov/arcgis/rest/services/Property/MD_Property_v1/MapServer/0/query?where=TRUE_SITE_ADDR%20LIKE%20'${encodeURIComponent('%' + addr.toUpperCase() + '%')}'&outFields=*&f=json&resultRecordCount=5`,
    parse: (a) => ({
...
async function lookupOwner({ address, city, state }, db) {
  ...
  // Honor requires_token (e.g. Maricopa) — graceful no-op if env unset
  if (ep.requires_token && !process.env[ep.requires_token]) {
    return { error: `requires_${ep.requires_token}`, county: key, deferred: true };
  }
  try {
    const opts = { ...FETCH_OPTS };
    if (ep.requires_token && process.env[ep.requires_token]) {
      opts.headers = { ...opts.headers, 'AUTHORIZATION': `Bearer ${process.env[ep.requires_token]}` };
    }
