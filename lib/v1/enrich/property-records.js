...
const COUNTY_ENDPOINTS = {
  ...
  // Fulton County, GA (Atlanta) — Fulton GIS hub
  'GA:Fulton': {
    url: (addr) => `https://gisdata.fultoncountyga.gov/server/rest/services/Hosted/Parcels/FeatureServer/0/query?where=SITUS_ADDR%20LIKE%20'${encodeURIComponent('%' + addr.toUpperCase() + '%')}'&outFields=*&f=json&resultRecordCount=5`,
    parse: (a) => ({
  ...
  // Miami-Dade County, FL — public assessor lightweight search API
  'FL:MiamiDade': {
    url: (addr) => `https://www.miamidade.gov/PaPortal/data/AddressSearch.aspx?address=${encodeURIComponent(addr)}`,
    parse: (a) => ({
      owner_name: a.Owner1 || a.OwnerName,
      mailing_address: a.MailingAddress,
      parcel_id: a.FolioNumber,
      assessed_value: a.TotalAssessedValue,
      year_built: a.YearBuilt,
      classification: a.PrimaryUse || 'unknown'
    }),
    type: 'json_array',
    extract: (data) => Array.isArray(data) ? data : (data?.results || [])
  },
  ...
  // Maricopa County, AZ (Phoenix) — official public API
  'AZ:Maricopa': {
...
async function lookupOwner({ address, city, state }, db) {
  ...
  try {
    const r = await fetch(ep.url(address), FETCH_OPTS);
