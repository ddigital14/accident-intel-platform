...
const COUNTY_ENDPOINTS = {
  ...
  // Los Angeles County, CA — Assessor public ArcGIS feature service
  // Phase 44B: free public lookup, no key needed
  'CA:LosAngeles': {
    url: (addr) => `https://maps.assessor.lacounty.gov/GVH_2_2/REST/services/PAIS_AddressSearch/MapServer/0/query?where=SitusFullAddress%20LIKE%20'${encodeURIComponent('%' + addr.toUpperCase() + '%')}'&outFields=*&f=json&resultRecordCount=5`,
    parse: (a) => ({
      owner_name: a.OwnerName || a.OWNER || a.OWNER1,
      mailing_address: a.MailAddress || [a.MAIL_ADDR, a.MAIL_CITY, a.MAIL_STATE, a.MAIL_ZIP].filter(Boolean).join(', '),
      parcel_id: a.AIN || a.APN,
      assessed_value: a.TotalValue || a.NetTaxableValue,
      year_built: a.YearBuilt || a.YEAR_BUILT,
      classification: a.UseType || a.LandUse || 'unknown'
    }),
    type: 'arcgis'
  },
  ...
  'AZ:Maricopa': {
    ...
    url: (addr) => `https://mcassessor.maricopa.gov/search/property/?q=${encodeURIComponent(addr)}`,
    parse: (a) => ({
      owner_name: typeof a.Ownership === 'string' ? a.Ownership : (a.Ownership?.Owner || a.owner_name),
      mailing_address: [a.SitusAddress, a.SitusCity, a.SitusZip].filter(Boolean).join(', ') || a.MailingAddress?.Address1 || a.mailing_address,
      ...
      subdivision: a.SubdivisonName || null,
      mcr: a.MCR || null
    }),
    type: 'maricopa',
    extract: (data) => {
...
function getCountyKey(state, city) {
  ...
  const map = {
    ...
    'CA:LOS ANGELES':       'CA:LosAngeles',
    'CA:LONG BEACH':        'CA:LosAngeles',
    'CA:GLENDALE':          'CA:LosAngeles',
    'CA:SANTA MONICA':      'CA:LosAngeles',
    'CA:PASADENA':          'CA:LosAngeles',
    'CA:TORRANCE':          'CA:LosAngeles',
    'CA:INGLEWOOD':         'CA:LosAngeles',
    'CA:BEVERLY HILLS':     'CA:LosAngeles',
    'CA:COMPTON':           'CA:LosAngeles',
    'CA:BURBANK':           'CA:LosAngeles',
    // Arizona
...
async function lookupOwner({ address, city, state }, db) {
  ...
  try {
    ...
    if (ep.requires_token && resolvedToken) {
      opts.headers = { ...opts.headers, 'AUTHORIZATION': resolvedToken };
    }
    ...
    let matches = arr.slice(0, 5).map(a => ep.parse(a));

    // Phase 36: Maricopa 2-pass enrich — search returns APN; fetch owner-details for richer data
    // (mailing address, deed date/type, sale price+date — gold for PI lead enrichment)
    if (ep.type === 'maricopa' && resolvedToken && matches.length) {
      const top = matches.slice(0, 3); // limit to top 3 to stay under time budget
      await Promise.all(top.map(async (m, idx) => {
        if (!m.parcel_id) return;
        try {
          const od = await fetch(`https://mcassessor.maricopa.gov/parcel/${encodeURIComponent(m.parcel_id)}/owner-details`, {
            headers: { 'AUTHORIZATION': resolvedToken, 'Accept': 'application/json', 'User-Agent': '' },
            signal: AbortSignal.timeout(10000)
          });
          if (od.ok) {
            const d = await od.json().catch(() => null);
            if (d && typeof d === 'object') {
              matches[idx] = {
                ...m,
                owner_name: d.Ownership || m.owner_name,
                mailing_address: d.FullMailingAddress || m.mailing_address,
                in_care_of: d.InCareOf || null,
                deed_number: d.MostCurrentDeed || null,
                deed_date: d.DeedDate || null,
                deed_type: d.DeedType || null,
        ...
        } catch (_) { /* graceful — keep base match */ }
      }));
    }
    return { matches, county: key };
  } catch (e) {
