...
/**
 * Extract relatives from an obituary text blob using GPT-4o-mini.
 * Returns { relatives: [{name, relation, age, city}], cost_usd }
 */
async function extractRelatives(db, obitText, deceasedName) {
  if (!OPENAI_API_KEY || !obitText) return { ok: false, error: 'no_openai_or_text' };
  const txt = String(obitText).substring(0, 4000);
  const prompt = `Obituary for ${deceasedName || 'the deceased'}:
"""
${txt}
"""

Extract every named relative from "survived by" / "preceded in death by" /
"leaves behind" / "loving (husband|wife|father|mother|son|daughter)" passages.
...
async function processDeceased(db, person) {
  ...
  // Find linked obituary
  const obit = await db('source_reports')
    .where('incident_id', person.incident_id)
    .where('source_type', 'obituary')
    .orderBy('created_at', 'desc')
    .first();
  if (!obit) return { ok: true, skipped: 'no_obituary' };

  // Pull text from raw_data or parsed_data
  let obitText = '';
  try {
    const raw = typeof obit.raw_data === 'string' ? JSON.parse(obit.raw_data) : obit.raw_data;
    obitText = raw?.html_excerpt || raw?.text || raw?.body || '';
    if (!obitText) {
      const parsed = typeof obit.parsed_data === 'string' ? JSON.parse(obit.parsed_data) : obit.parsed_data;
      obitText = parsed?.full_text || JSON.stringify(parsed || {}).substring(0, 3000);
    }
  } catch (_) { obitText = String(obit.raw_data || obit.parsed_data || ''); }

  if (!obitText || obitText.length < 80) return { ok: true, skipped: 'thin_obit' };

  const result = await extractRelatives(db, obitText, person.full_name);
