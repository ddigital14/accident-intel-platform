  ...
  if (!logs.length) {
    ...
    try {
      await db('enrichment_logs').insert({
        ...
        new_value: JSON.stringify({ matches: 0, conflicts: 0, detail: ['no_evidence_yet'], source: 'evidence-cross-checker' }).slice(0, 4000),
        created_at: new Date()
      });
    } catch (_e) { console.error('[xcheck] no_evidence_yet insert failed:', _e.message); result._insert_err = _e.message; }
  ...
  if (result.matches > 0 || result.conflicts > 0) {
    ...
    try {
      await db('enrichment_logs').insert({
        ...
        field_name: 'evidence_cross_check_summary',
        new_value: JSON.stringify({
          matches: result.matches, conflicts: result.conflicts, detail: result.detail,
          source: 'evidence-cross-checker', cross_engine_conflict: result.conflicts > 0
        }).slice(0, 4000),
        created_at: new Date()
      });
    } catch (_) {}
