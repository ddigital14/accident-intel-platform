...
const HARD_BAN_NAMES = new Set([
  ...
  // Phase 65: celebrity false-positives caught in Phase 64 sweep
  'lindsey vonn',     // alpine skier — name pulled from articles where she was mentioned
  'diogo jota',       // Liverpool footballer (FC) — pulled from sports articles
  'greg biffle',      // NASCAR driver — pulled from racing articles
  'tom brady', 'taylor swift', 'elon musk', 'donald trump', 'joe biden',
  'kamala harris', 'tiger woods', 'serena williams', 'lebron james',
  'cristiano ronaldo', 'lionel messi', 'patrick mahomes', 'travis kelce',
  'caitlin clark', 'angel reese', 'simone biles', 'dale earnhardt',
  // Phase 66c: extra non-victim names caught in merge proposals
  'unknown', 'unknown unknown', 'u.s. federal agent', 'us federal agent',
  'federal agent', 'officer', 'hpd officer', 'unknown faulkner cousin',
  'officer 1', 'officer 2', 'officer 3', 'officer 4',
  'kyle larson', 'denny hamlin', 'kyle busch', 'kevin harvick',
  'bubba wallace', 'chase elliott', 'william byron', 'erling haaland',
  'kevin de bruyne', 'mohamed salah', 'kylian mbappe', 'neymar jr',
  'novak djokovic', 'rafael nadal', 'roger federer', 'ja morant',
  'stephen curry', 'kevin durant', 'giannis antetokounmpo', 'nikola jokic',
  'luka doncic', 'shohei ohtani', 'mike trout', 'aaron judge',
  'patrick mahomes', 'josh allen', 'jalen hurts', 'lamar jackson',
  'christian mccaffrey', 'derrick henry', 'aaron rodgers'
]);

// Phase 65: pattern-based celebrity detection — names paired with sports/entertainment context
// in surrounding text are auto-rejected (catches NEW celebrities not in HARD_BAN_NAMES)
const CELEBRITY_CONTEXT_RX = /\b(NASCAR|Formula\s*1|F1|Indy\s*500|NBA|NFL|MLB|NHL|MLS|Premier\s*League|Liverpool|Manchester|Real\s*Madrid|Barcelona|Lakers|Warriors|Celtics|Yankees|Dodgers|Patriots|Eagles|49ers|Cowboys|Olympic|Olympics|Grammy|Oscar|Emmy|Tony\s*Award|Hall\s*of\s*Fame|All-Star|MVP|championship|world\s*cup|World\s*Cup|Super\s*Bowl|Stanley\s*Cup|World\s*Series|Wimbledon|US\s*Open|grand\s*slam|Grand\s*Slam|Tour\s*de\s*France|Indianapolis\s*500|Daytona\s*500|Kentucky\s*Derby)\b/;
...
function applyDenyList(name, surroundingText) {
  ...
  // Phase 65: if name appears in obvious sports/entertainment context, reject
  if (surroundingText && CELEBRITY_CONTEXT_RX.test(String(surroundingText))) {
    // Only reject if the name is mentioned NEAR (within 300 chars) the celebrity context word
    const ctxMatch = String(surroundingText).match(CELEBRITY_CONTEXT_RX);
    if (ctxMatch) {
      const ctxIdx = ctxMatch.index;
      const nameIdx = String(surroundingText).toLowerCase().indexOf(norm);
      if (nameIdx >= 0 && Math.abs(nameIdx - ctxIdx) < 300) return null;
    }
  }
...
function quickClassify(name, text) {
  ...
  // Phase 38 v2: VICTIM-VERB-DOMINANCE — if the name appears within 250 chars of a victim verb
  // (killed, slain, fallen, dead, died, injured, struck, hit), they're a victim REGARDLESS of
  // any byline/official-title proximity. This catches "fallen Deputy Heather Avery" and
  // "journalist Ray Hartmann killed" — the title is descriptive, not disqualifying.
  let victimVerbDominant = false;
  if (VICTIM_VERBS_RX.test(t)) {
    ...
    if (idx >= 0) {
      ...
      if (VICTIM_VERBS_RX.test(window)) {
        victimVerbDominant = true;
      }
    }
  }
  if (victimVerbDominant) {
    return { decision: 'accept', confidence: 92, reason: 'victim_verb_near_name' };
  }
