...
// Phase 22: legacy.com 403's our UA. tributes.com has comparable coverage
// and accepts standard UA. Try tributes.com first; fall back to legacy.com.
const TRIBUTES_BASE = 'https://www.tributes.com/obituaries/search';
const LEGACY_BASE   = 'https://www.legacy.com/us/obituaries/search';

function buildTributesUrl(city, state) {
  return `${TRIBUTES_BASE}?searchword=&city=${encodeURIComponent(city)}&state=${encodeURIComponent(state)}`;
}
...
async function fetchObitHtml(db, city, state) {
  ...
  try {
    const url = buildTributesUrl(city, state);
    ...
    if (r.ok) {
      await trackApiCall(db, 'obit-backfill', 'tributes_com', 0, 0, true);
      return (await r.text()).substring(0, 80000);
    }
    await trackApiCall(db, 'obit-backfill', 'tributes_com', 0, 0, false).catch(()=>{});
  } catch (_) {}

  // Fallback: legacy.com (may 403 — tolerated)
  try {
    ...
    if (!r.ok) {
      await trackApiCall(db, 'obit-backfill', 'legacy_com', 0, 0, false).catch(()=>{});
      return null;
    }
