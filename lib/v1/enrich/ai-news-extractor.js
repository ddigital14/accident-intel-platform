...
const { extract, extractJson } = require('./_ai_router');
...
async function extractFromIncident(db, incidentId) {
  ...
  const aiResult = await extract(db, {
  ...
  if (!aiResult || !aiResult.ok) {
    return { ok: false, error: 'ai_call_failed', incident_id: incidentId, ai_error: aiResult?.error, attempts: aiResult?.attempts };
  }
  ...
  if (!parsed || !Array.isArray(parsed.victims)) {
    return {
      ...
      raw_preview: String(aiResult.content || '').slice(0, 400),
