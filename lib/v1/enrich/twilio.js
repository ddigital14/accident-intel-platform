...
async function lookupPhone(db, phone, opts = {}) {
  ...
  const fields = opts.fields || 'line_type_intelligence,caller_name,sim_swap';
  ...
  try {
    ...
    return {
      ...
      sim_swap: data.sim_swap || null,
      sim_last_swap_date: data.sim_swap?.last_sim_swap?.last_sim_swap_date || null,
      sim_swapped_period: data.sim_swap?.last_sim_swap?.swapped_period || null,
      sim_recently_swapped: !!(data.sim_swap?.last_sim_swap?.swapped_in_period_30_days),
      raw: data
    };
  } catch (e) {
...
async function applyLookupToPerson(db, personId, lookup, sourceUrl = null) {
  ...
  if (lookup.sim_recently_swapped) {
    updates.phone_recently_swapped = true;
    // identity confidence penalty: -30 (carrier flagged a recent SIM swap)
  }
  if (/voip/i.test(String(lookup.line_type || ''))) {
    updates.phone_is_voip = true;
  }
  ...
  // Phase 44B: SIM-swap penalty + VoIP penalty on identity_confidence
  try {
    const ex = await db('persons').where('id', personId).first();
    let penalty = 0;
    if (lookup.sim_recently_swapped) penalty -= 30;
    if (/voip/i.test(String(lookup.line_type || ''))) penalty -= 10;
    if (penalty < 0 && ex) {
      const cur = ex.identity_confidence ?? ex.confidence_score ?? 50;
  ...
  // Phase 44B: store phone_intelligence in enrichment_logs.meta
  try {
    await db('enrichment_logs').insert({
      person_id: personId,
      field_name: 'phone_intelligence',
      new_value: JSON.stringify({
        line_type: lookup.line_type,
        carrier: lookup.carrier_name,
        recently_swapped: !!lookup.sim_recently_swapped,
        last_swap_date: lookup.sim_last_swap_date || null,
        swapped_period: lookup.sim_swapped_period || null,
        is_voip: /voip/i.test(String(lookup.line_type || ''))
      }).slice(0, 1500),
      confidence: 80,
      created_at: new Date()
    });
  } catch (_) { /* meta column may differ; soft fail */ }
...
async function compliance(db) {
  ...
  try {
    ...
    if (r.ok) { const j = await r.json(); out.numbers = (j.incoming_phone_numbers || []).map(n => ({ sid: n.sid, phone: n.phone_number, address_sid: n.address_sid, emergency_address_sid: n.emergency_address_sid, emergency_status: n.emergency_status, has_emergency_address: !!(n.emergency_address_sid), sms_url: n.sms_url, voice_url: n.voice_url })); }
  } catch (_) {}
...
async function provisionCompliance(db, opts = {}) {
  ...
  if (opts.attach_address_sid && opts.phone_sid) {
    const p2 = new URLSearchParams({ AddressSid: opts.attach_address_sid, EmergencyAddressSid: opts.attach_address_sid, EmergencyStatus: 'Active' });
    ...
    if (nr.ok) {
      ...
      for (const n of (nj.incoming_phone_numbers || [])) {
        const p2 = new URLSearchParams({ AddressSid: out.address.sid, EmergencyAddressSid: out.address.sid, EmergencyStatus: 'Active' });
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'sim_check') {
      // Phase 44B: dedicated SIM-swap sweep — re-runs lookup with sim_swap
      // field on persons whose phone hasn't been intelligence-checked.
      const limit = parseInt(req.query.limit) || 25;
      const result = await simSwapBatch(db, limit);
      return res.status(200).json({ success: true, ...result });
    }
    ...
    return res.status(400).json({ error: 'unknown action', valid: ['health', 'lookup', 'enrich_pending', 'sim_check', 'send_sms', 'send_mms', 'call', 'compliance', 'provision_compliance'] });
  } catch (e) {
...
// -------- SIM-SWAP BATCH (Phase 44B) --------
async function simSwapBatch(db, limit = 25) {
  const candidates = await db('persons')
    .whereNotNull('phone')
    .orderBy('updated_at', 'desc')
    .limit(limit);
  let checked = 0; let swapped = 0; let voip = 0;
  for (const p of candidates) {
    const r = await lookupPhone(db, p.phone, { fields: 'line_type_intelligence,sim_swap' });
    if (!r.ok) continue;
    checked++;
    if (r.sim_recently_swapped) swapped++;
    if (/voip/i.test(String(r.line_type || ''))) voip++;
    await applyLookupToPerson(db, p.id, r).catch(() => {});
  }
  return { checked, recently_swapped: swapped, voip };
}
...
module.exports.simSwapBatch = simSwapBatch;
