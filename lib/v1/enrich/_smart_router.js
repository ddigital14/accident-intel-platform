...
function pickNextAction(person, incident, identityConfidence = null) {
  ...
  if (!person.phone && (person.full_name || person.last_name) && !person.has_voter_searched) {
    return { action: 'enrich-voter-rolls', reason: 'name_but_no_phone_try_free_voter_first', params: { person_id: person.id }, cost_estimate_usd: 0, ready_for_rep: false };
  }
  ...
  try {
    switch (picked.action) {
      ...
      case 'enrich-pdl-by-name': {
        // Phase 22: actually execute PDL-by-name inline now that handler exists
        try {
          const pdlbn = require('./pdl-by-name');
          if (typeof pdlbn.processPerson === 'function') {
            result = await pdlbn.processPerson(db, person);
          } else {
