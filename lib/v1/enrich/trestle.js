...
async function callTrestle(path, params = {}, db = null) {
  ...
  try {
    ...
    const success = r.ok;
    const svcNames = {
    '/3.2/phone': 'trestle_reverse_phone',
    '/3.1/cnam': 'trestle_cnam',
    '/3.2/location': 'trestle_reverse_address',
    '/3.2/caller_id': 'trestle_caller_id',
    '/3.0/real_contact': 'trestle_real_contact',
    '/3.0/phone_intel': 'trestle_phone_intel'
  };
  const svcName = svcNames[path] || 'trestle_other';
  if (db) await trackApiCall(db, 'trestle', svcName, 0, 0, success).catch(() => {});
...
async function reverseName({ name_first, name_last, address_street, address_city, address_state, address_postal_code }, db) {
  return await callTrestle('/3.1/person', { name: { first: name_first, last: name_last }, address: { street_line_1: address_street, city: address_city, postal_code: address_postal_code, state_code: address_state, country_code: 'US' } }, db);
}

module.exports = {
  isConfigured, getApiKey,
  reversePhone, smartCnam, reverseAddress, callerIdentification, realContact,
  enrichPersonViaTrestle, reverseName,
};
