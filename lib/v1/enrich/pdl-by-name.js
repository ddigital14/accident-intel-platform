...
async function callPdlByName(person) {
  ...
  // Path 2: ENRICH endpoint (Pro plan with credits) — returns best-match for name+locality+region inputs
  // Phase 47: switched from /search (Free tier exhausted) to /enrich (Pro tier, 442/450 credits remaining)
  const params = new URLSearchParams();
  if (first) params.append('first_name', first);
  if (last)  params.append('last_name',  last);
  if (person.city)  params.append('locality', person.city);
  ...
  try {
    ...
    if (!r.ok) return { ok: false, status: r.status, mode: 'enrich-name' };
    const d = await r.json();
    if (d.status !== 200 || !d.data) return { ok: false, status: d.status || 'no_match', mode: 'enrich-name' };
    return { ok: true, data: d.data, likelihood: d.likelihood, mode: 'enrich-name' };
