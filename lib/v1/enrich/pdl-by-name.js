...
/**
 * Call PDL Person Enrichment for a (name, locality, region) tuple.
 * Mirrors deep.js#pdlEnrich call pattern.
 */
async function callPdlByName(person) {
  ...
  params.append('min_likelihood', '4');

  try {
    const r = await fetch(`${PDL_URL}?${params}`, {
      headers: { 'X-API-Key': PDL_KEY, 'Accept': 'application/json' },
      signal: AbortSignal.timeout(TIMEOUT_MS)
    });
    if (!r.ok) return { ok: false, status: r.status };
    const d = await r.json();
    if (d.status !== 200 || !d.data) return { ok: false, status: d.status, likelihood: d.likelihood };
    return { ok: true, data: d.data, likelihood: d.likelihood };
  } catch (e) {
