...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'batch') {
      ...
      return res.json({
        success: true,
        message: `Identity investigator: ${out.upgraded}/${out.evaluated} upgraded, ${out.fields_filled} fields filled`,
        ...out,
      });
    }
    return res.status(400).json({ error: 'invalid action', valid: ['batch', 'person', 'health'] });
  } catch (err) {
