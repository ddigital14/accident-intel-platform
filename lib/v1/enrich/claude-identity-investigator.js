...
// Phase 24 #5 — Backfill identity_confidence for persons with NULL via cross-exam
async function backfillIdentityConfidence(db, opts = {}) {
  const limit = Math.min(opts.limit || 200, 500);
  const out = { evaluated: 0, updated: 0, skipped: 0, total_null_before: 0, samples: [] };
  try {
    ...
    for (const p of rows) {
      ...
      try {
        ...
        const conf = r && typeof r.identity_confidence === 'number' ? r.identity_confidence : null;
        if (conf == null) {
          // fallback to person.confidence_score baseline
          const fb = parseInt(p.confidence_score) || 50;
          await db('persons').where('id', p.id).update({
            identity_confidence: fb,
            updated_at: new Date()
          });
          out.updated++;
          if (out.samples.length < 5) out.samples.push({ id: p.id, name: p.full_name, ic: fb, source: 'fallback' });
          continue;
        }
        await db('persons').where('id', p.id).update({
        ...
        out.updated++;
        if (out.samples.length < 5) out.samples.push({ id: p.id, name: p.full_name, ic: conf, source: 'cross-exam' });
      } catch (e) {
        out.skipped++;
      }
    }
