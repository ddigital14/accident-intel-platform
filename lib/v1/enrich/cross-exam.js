  ...
  try {
    const r = await db.raw(`
      SELECT field_name, new_value,
             COALESCE(source, source_url, 'enrichment') as source,
             confidence
      FROM enrichment_logs WHERE person_id = ?
    `, [person.id]);
    logs = r.rows || [];
  } catch (_) {
    // fallback to safe minimal columns
    try {
      logs = await db('enrichment_logs').where('person_id', person.id).select('field_name', 'new_value', 'confidence');
    } catch (_) { logs = []; }
  }
  for (const l of logs) {
