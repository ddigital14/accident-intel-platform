...
async function gatherEvidence(db, person) {
  ...
  let logs = [];
  // Try multiple schema variants — different deploys have different columns
  const queryAttempts = [
    `SELECT field_name, new_value, COALESCE(source, source_url, 'enrichment') as source, confidence FROM enrichment_logs WHERE person_id = ?`,
    `SELECT field_name, new_value, source_url as source, confidence FROM enrichment_logs WHERE person_id = ?`,
    `SELECT field_name, new_value, 'enrichment' as source, confidence FROM enrichment_logs WHERE person_id = ?`,
    `SELECT field_name, new_value, 'enrichment' as source FROM enrichment_logs WHERE person_id = ?`
  ];
  for (const q of queryAttempts) {
    try {
      const r = await db.raw(q, [person.id]);
      logs = r.rows || [];
      break;
