...
async function verify(name, text, meta = {}) {
  ...
  const HISTORICAL_PATTERNS = [
    /\b\d+\s*years?\s*(ago|later|after|since)\b/,  // any number of years
...
async function batchVerify(db, { limit = 20 } = {}) {
  ...
  for (const r of rows) {
    ...
    if (v.is_victim) {
      ...
      // Phase 40: event-driven contact-finder.  auto_resolve is action-routed to
      // victim-contact-finder by lib/v1/system/_cascade.js so reps don't wait for cron.
      try { await enqueueCascade(db, { person_id: r.pid, incident_id: r.incident_id, trigger_source: 'auto_resolve', priority: 8 }); } catch (_) {}
      // Fatal incidents → also queue family-from-obit pull
      try {
        if (r.inc_type && /fatal|dead|killed|deceased/i.test(String(r.inc_type))) {
          await enqueueCascade(db, { person_id: r.pid, incident_id: r.incident_id, trigger_source: 'fatal_family', priority: 7 });
        }
      } catch (_) {}
    } else {
      results.rejected++;
