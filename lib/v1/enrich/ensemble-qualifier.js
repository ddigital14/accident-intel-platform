...
const QUALIFY_THRESHOLD = 120;  // Phase 37: lowered 180->120 to clear pending_named backlog
...
async function run(db, limit = 50) {
  ...
  const sql = `
    ...
    HAVING COALESCE(SUM(weight_n), 0) >= ${QUALIFY_THRESHOLD}
  ...
  return { candidates: rows.length, promoted, threshold: QUALIFY_THRESHOLD };
}

module.exports = async function handler(req, res) {
  const db = getDb();
  try {
    if (req.query?.action === 'health') return res.json({ ok: true, engine: 'ensemble-qualifier', threshold: QUALIFY_THRESHOLD });
