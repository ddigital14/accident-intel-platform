...
async function reasonWithOpus(db, ctx) {
  const sys = 'You are an expert OSINT analyst. Given accident scenario data, reason about likely missing contact info. Use phone-numbering geography (mobile vs landline patterns), surname/age/role to infer family relationships, and corporate email conventions. NEVER fabricate phone numbers or emails - only suggest patterns/area codes/types and label confidence honestly. Return JSON only.';
