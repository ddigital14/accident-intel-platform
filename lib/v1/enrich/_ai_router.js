...
const MODELS = {
  ...
  premium_anth: 'claude-opus-4-5-20250929',  // Phase 43: latest Anthropic flagship API model ID. Override via system_config 'model_registry' or model_registry table to flip without redeploy.
};
const TASK_MAP = {
...
async function extract(db, {
  ...
  for (const p of [primary, fallback].filter(Boolean)) {
    ...
    attempts.push({ provider: p, ok: r.ok, error: r.error, status: r.status });
