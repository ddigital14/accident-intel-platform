...
const MODELS = {
  ...
  premium_anth: 'claude-opus-4-7',  // Phase 43: upgraded 4.6 -> 4.7 (Anthropic flagship as of 2026-04-28)
};
const TASK_MAP = {
...
async function extract(db, {
  ...
  for (const p of [primary, fallback].filter(Boolean)) {
    ...
    if (p === 'openai') {
      ...
      const claudeModel = (model === MODELS.premium_oai) ? MODELS.mid_anth : MODELS.cheap_anth;
