...
async function deepEnrichPerson(person, db) {
  ...
  if (pdl) {
    ...
    if (!out.merged_fields.address && pdl.location_street_address) out.merged_fields.address = pdl.location_street_address;
    // Expanded PDL fields — store in merged for richer profile
    if (pdl.industry) out.merged_fields.industry = pdl.industry;
    if (pdl.job_company_industry) out.merged_fields.employer_industry = pdl.job_company_industry;
    if (pdl.job_company_website) out.merged_fields.employer_domain = pdl.job_company_website.replace(/^https?:\/\//, '').split('/')[0];
    if (pdl.education?.length) out.merged_fields.education = pdl.education.map(e => e.school?.name).filter(Boolean).join('; ');
    if (pdl.skills?.length) out.merged_fields.skills = pdl.skills.slice(0, 10).join('; ');
    if (pdl.experience?.length > 1) out.merged_fields.work_history_count = pdl.experience.length;
    if (pdl.profiles?.length) {
      const fb = pdl.profiles.find(pr => pr.network === 'facebook');
      const tw = pdl.profiles.find(pr => pr.network === 'twitter');
      if (fb && !out.merged_fields.facebook_url) out.merged_fields.facebook_url = fb.url;
      if (tw && !out.merged_fields.twitter_url) out.merged_fields.twitter_url = tw.url;
    }
    if (pdl.facebook_url && !out.merged_fields.facebook_url) out.merged_fields.facebook_url = pdl.facebook_url;
    if (pdl.twitter_url && !out.merged_fields.twitter_url) out.merged_fields.twitter_url = pdl.twitter_url;
    if (pdl.github_url) out.merged_fields.github_url = pdl.github_url;
    // Personal emails (better than work email for non-business reach)
    if (pdl.personal_emails?.length && !out.merged_fields.email) out.merged_fields.email = pdl.personal_emails[0];
    if (pdl.phone_numbers?.length) {
      // Save extra phones
      const extras = pdl.phone_numbers.filter(p => p !== out.merged_fields.phone);
      if (extras.length && !out.merged_fields.phone_secondary) out.merged_fields.phone_secondary = extras[0];
    }
  }
  ...
  // STEP 8: Twilio Lookup (carrier+caller_name+line_type) — free identity confidence boost
  if ((out.merged_fields.phone || person.phone) && !out.merged_fields.caller_name) {
    try {
      const tw = require('./twilio');
      const phone = out.merged_fields.phone || person.phone;
      const lu = await tw.lookupPhone(db, phone);
      if (lu?.ok) {
        out.evidence.twilio_lookup = { line_type: lu.line_type, carrier: lu.carrier_name, caller_name: lu.caller_name };
        out.sources_used.push('twilio_lookup');
        out.cost_estimate_usd = (out.cost_estimate_usd || 0) + 0.008;
        if (lu.caller_name && !out.merged_fields.full_name) out.merged_fields.full_name = lu.caller_name;
        if (lu.carrier_name && !out.merged_fields.phone_carrier) out.merged_fields.phone_carrier = lu.carrier_name;
        if (lu.line_type && !out.merged_fields.phone_line_type) out.merged_fields.phone_line_type = lu.line_type;
        if (lu.valid != null && out.merged_fields.phone_verified === undefined) out.merged_fields.phone_verified = !!lu.valid;
      }
    } catch (_) {}
  }

  // STEP 9: Voter rolls (FREE) — when we have name + state
  const fname = out.merged_fields.first_name || person.first_name;
  const lname = out.merged_fields.last_name || person.last_name;
  const pstate = out.merged_fields.state || person.state;
  if (fname && lname && pstate && !out.merged_fields.address) {
    try {
      const vr = require('./voter-rolls');
      if (vr.lookupVoter) {
        const matches = await vr.lookupVoter(db, fname, lname, pstate);
        if (matches?.length) {
          out.evidence.voter_rolls = { matches: matches.length };
          out.sources_used.push('voter_rolls');
          const v = matches[0];
          if (v.residence_address && !out.merged_fields.address) out.merged_fields.address = v.residence_address;
          if (v.year_of_birth && !out.merged_fields.age) out.merged_fields.age = new Date().getFullYear() - v.year_of_birth;
        }
      }
    } catch (_) {}
  }

  // STEP 10: Adjacent records — find obituaries / news / court records within 7d/50mi (FREE)
  if (person.incident_id) {
    try {
      const { findAdjacentRecords } = require('./cross-exam');
      const inc = await db('incidents').where('id', person.incident_id).first();
      if (inc) {
        const adj = await findAdjacentRecords(db, inc);
        if (adj && adj.length) {
          out.evidence.adjacent_records = adj.slice(0, 5).map(a => ({ source: a.source_type, address: a.address }));
          out.sources_used.push('adjacent_records');
        }
      }
    } catch (_) {}
  }
