...
module.exports = async function handler(req, res) {
  ...
  try {
    const candidates = await db('persons as p')
      ...
      .where(function () { this.whereIn('i.qualification_state', ['pending', 'pending_named']).orWhereNull('i.qualification_state'); })
      .where(function () { this.whereNull('p.phone').orWhereNull('p.email'); })
      .where('p.created_at', '>', new Date(Date.now() - 30 * 86400000))
      .select('p.id', 'p.full_name', 'p.city', 'p.state', 'p.phone', 'p.email')
