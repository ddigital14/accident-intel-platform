...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Phase 24 #3: Re-rotate stale assignments
    // If a qualified lead has been assigned >7 days AND status still new/unclaimed -> reassign
    // If status='declined' -> reassign immediately
    const STALE_DAYS = parseInt(req.query.stale_days) || 7;
    const staleCutoff = new Date(Date.now() - STALE_DAYS * 86400 * 1000);
    let unclaimedRereleased = 0, declinedReleased = 0;
    try {
      const r1 = await db('incidents')
        .where('qualification_state', 'qualified')
        .whereNotNull('assigned_to')
        .where('assigned_at', '<', staleCutoff)
        .whereIn('status', ['new', 'unclaimed', 'assigned'])
        .update({
          assigned_to: null,
          assigned_at: null,
          status: 'new',
      ...
      unclaimedRereleased = r1 || 0;
    } catch (_) {}
    try {
      const r2 = await db('incidents')
        .where('qualification_state', 'qualified')
        .whereNotNull('assigned_to')
      ...
      declinedReleased = r2 || 0;
    } catch (_) {}
    results.unclaimed_released = unclaimedRereleased;
    results.declined_released = declinedReleased;
    ...
    const candidates = await db('incidents')
      ...
      .select('id','metro_area_id','incident_type','lead_score','severity','description')
    ...
    for (const inc of candidates) {
      try {
        // Phase 66: apply pattern-miner signal delta before assignment
        try {
          const pm = require('./pattern-miner');
          const r = await pm.applySignals(db, inc.id);
          if (r?.ok && typeof r.adjusted_score_delta === 'number') {
            inc._adjusted_score = Math.max(0, Math.min(100, (inc.lead_score || 0) + r.adjusted_score_delta));
            inc._pattern_delta = r.adjusted_score_delta;
          } else { inc._adjusted_score = inc.lead_score; }
        } catch (_) { inc._adjusted_score = inc.lead_score; }
        // Find best rep
        ...
        await db('activity_log').insert({
          ...
          details: JSON.stringify({
            lead_score: (inc._adjusted_score || inc.lead_score),
    ...
    res.json({
      success: true,
      message: `Auto-assign: ${results.assigned} assigned, ${results.unclaimed_released||0} stale released, ${results.declined_released||0} declined released, ${results.candidates} candidates`,
      ...results,
