...
async function runRecipe(db, personId, recipeId) {
  ...
  // Delegate to auto-fan-out's runFanOut for actual engine execution
  ...
  // For now, run the full fan-out — the recipe just drove which we'd label this run as
  const result = await fanOut.runFanOut(db, personId, { trigger_field: 'recipe:' + recipeId, force: true });
  return { ok: true, recipe_id: recipeId, fan_out_result: result };
}
