...
async function synthesizeOne(db, personId) {
  ...
  // Fallback path: if Claude rejects (e.g. model name mismatch returns 400),
  // fall back to GPT-4o premium so reps still get a synthesis.
  if (!r.ok) {
    r = await extract(db, {
      pipeline: 'best-lead-synthesizer:fallback',
      systemPrompt: SYNTH_SYSTEM,
      userPrompt,
      provider: 'openai',
      tier: 'premium',
