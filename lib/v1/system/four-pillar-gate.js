...
async function pillar2_interface(db, surface) {
  ...
  try {
    if (surface === 'mobile-card' || surface === 'desktop-detail' || surface === 'master-list-row' || surface === 'rep-handoff' || surface === 'map-view') {
      ...
      const count = r?.leads?.length || r?.count || 0;
      if (count === 0) return { ok: false, reason: 'master-lead-list returned 0 leads', count: 0 };
      return { ok: true, source: 'master-lead-list.gatherLeads', count };
    }
