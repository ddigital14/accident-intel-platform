...
async function findCandidateArticles(db, incident) {
  ...
  const rows = await db('incidents')
    ...
    .where(function() {
      this.whereNotILike('incident_number', 'nyc-opendata:%')
        .whereNotILike('incident_number', 'sf-datasf:%')
        .whereNotILike('incident_number', 'chicago-socrata:%')
        .whereNotILike('incident_number', 'la-opendata:%');
    })
    .limit(30);
  const candidates = [];
  for (const r of rows) {
