...
function extractPatterns(inc) {
  const keys = [];
  const desc = (inc.description || inc.summary || '').toLowerCase();
  const src = (inc.source_type || inc.discovery_source || '').toLowerCase();
...
async function mineSignals(db, lookbackDays = 90) {
  ...
  const incidents = await db('incidents')
    ...
    .select('id', 'discovered_at', 'state', 'description', 'summary', 'source_type', 'discovery_source', 'fatal_count');
