    ...
    if (req.query?.send === 'true' || req.query?.send === '1') {
      try {
        ...
        if (!recipients.length) {
          return res.json({ success: false, error: 'no_active_recipients' });
        }
        const reps = recipients.filter(u => u.role === 'rep').map(u => u.email);
        const admins = recipients.filter(u => u.role === 'admin' || u.role === 'manager').map(u => u.email);
        const all = [...new Set([...reps, ...admins])];
