...
async function gatherLeads(db) {
  const incidents = await db('incidents')
    .whereIn('qualification_state', ['qualified', 'pending_review'])
    ...
    if (primary?.id) {
      try {
        ...
        if (cc) {
          ...
          // Phase 55e: data column was removed — cross_engine_conflict is now embedded in new_value JSON
          crossCheck = {
            matches: d.matches || 0,
            conflicts: d.conflicts || 0,
            detail: Array.isArray(d.detail) ? d.detail : [],
            cross_engine_conflict: !!d.cross_engine_conflict
          };
        }
      } catch (_) {}
    }
    ...
    if (format === 'json') {
      return res.status(200).json({
        ...
        leads: r.leads.map(L => ({
          ...
          discrepancy_note: L.discrepancyNote,
          lead_tier: L.primary?.lead_tier || (L.incident.qualification_state === 'pending_review' ? 'review' : 'normal'),
          qualification_state: L.incident.qualification_state,
