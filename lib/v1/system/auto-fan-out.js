...
const ENGINE_MATRIX = [
  ...
  { id: 'victim-resolver', fires_on: ['name','always'], fills: ['phone','email','address'],
    adapter: async (db, p) => { const m = require('../enrich/victim-resolver'); return m.resolveOne ? m.resolveOne(db, p) : null; } },

  { id: 'victim-contact-finder', fires_on: ['name','always'], fills: ['phone','email','address'],
    adapter: async (db, p) => { const m = require('../enrich/victim-contact-finder'); return m.resolveOne ? m.resolveOne(db, p) : null; } },

  { id: 'osint-miner', fires_on: ['name','always'], fills: ['phone','email','social','employer'],
    adapter: async (db, p) => { const m = require('../enrich/homegrown-osint-miner'); return m.mineOne ? m.mineOne(db, p) : null; } },

  { id: 'deep-phone-research', fires_on: ['name'], fills: ['phone'],
    adapter: async (db, p) => { const m = require('../enrich/deep-phone-research'); return m.researchOne ? m.researchOne(db, p) : null; } },
...
async function runFanOut(db, personId, opts = {}) {
  ...
  const fanOutPromises = engines.map(async (spec) => {
    ...
    try {
      ...
      return { id: spec.id, status: skipped ? 'skipped' : 'ok', skipped, dur: Date.now() - t0, result: r ? (typeof r === 'object' ? Object.keys(r).slice(0, 6) : 'ok') : 'empty' };
    } catch (e) {
