...
const ENGINE_MATRIX = [
  ...
  { id: 'usps-validate', fires_on: ['address'], fills: ['zip4','address_canonical'],
    adapter: async (db, p) => {
      ...
      if (!p.address) return { skipped: 'no_address' };
      // Phase 66: address column is freeform string; parse into components for USPS
      let street = p.address, city = p.city || '', state = p.state || '', zip = p.zip || '';
      // Heuristic: if address contains commas, last token = state+zip, second-to-last = city
      if (!city || !state) {
        ...
        if (parts.length >= 3) {
          ...
          const m1 = tail.match(/^([A-Z]{2})\s+(\d{5}(-\d{4})?)?/i);
          if (m1) { state = state || m1[1].toUpperCase(); zip = zip || m1[2] || ''; }
          else { state = state || tail.slice(0, 2).toUpperCase(); }
        }
      }
      if (!street || !state) return { skipped: 'no_street_or_state' };
      return m.uspsValidate ? m.uspsValidate({ street, city, state, zip }, db) : null;
    } },

  // ── Phase 67: forward geocoder (Nominatim free) — runs AFTER usps-validate ──
  { id: 'geocoder', fires_on: ['address'], fills: ['lat','lon'],
    adapter: async (db, p) => {
      const m = require('../enrich/geocoder');
      if (!p.address) return { skipped: 'no_address' };
      if (p.lat != null && p.lon != null) return { skipped: 'already_geocoded' };
      return m.geocodeOne ? m.geocodeOne(db, p.id) : null;
    } },
  ...
  // ── Phase 92 engines ──
  { id: 'deep-dive-narrow', fires_on: ['name'], fills: ['phone','email','address','city'],
    adapter: async (db, p) => {
      // Only run if missing at least one contact field
      if (p.phone && p.email && p.address) return { skipped: 'fully_contacted' };
      const m = require('./deep-dive-narrow');
      const fakeReq = { method: 'POST', query: { secret: 'ingest-now', action: 'narrow', person_id: p.id }, headers: {} };
      const fakeRes = { _data: null, status() { return this; }, json(d) { this._data = d; return this; }, setHeader() {} };
      try { await m(fakeReq, fakeRes); return fakeRes._data; } catch { return null; }
    } },
];
...
async function handler(req, res) {
  ...
  if (action === 'force_remigrate') {
    _migrationApplied = false;
    await applyMigrationIdempotent(db);
    // Drop and recreate trigger explicitly to nuke cached function definition
    try {
      await db.raw('DROP TRIGGER IF EXISTS persons_auto_fan_out_trigger ON persons');
      await db.raw('DROP FUNCTION IF EXISTS enqueue_auto_fan_out() CASCADE');
      _migrationApplied = false;
      await applyMigrationIdempotent(db);
      return res.json({ success: true, remigrated: true });
    } catch (e) {
