  ...
  try {
    ...
    if (purgeSeeds) {
      ...
      if (allSeedIncIds.length) {
        const r1 = await db.raw('DELETE FROM source_reports WHERE incident_id = ANY(?::uuid[])', [allSeedIncIds]).catch(() => ({ rowCount: 0 }));
        reportsDeleted = r1.rowCount || 0;
        const r2 = await db.raw('DELETE FROM vehicles WHERE incident_id = ANY(?::uuid[])', [allSeedIncIds]).catch(() => ({ rowCount: 0 }));
        vehiclesDeleted = r2.rowCount || 0;
        const r3 = await db.raw('DELETE FROM persons WHERE incident_id = ANY(?::uuid[])', [allSeedIncIds]).catch(() => ({ rowCount: 0 }));
        personsDeleted = r3.rowCount || 0;
        const r4 = await db.raw('DELETE FROM incidents WHERE id = ANY(?::uuid[])', [allSeedIncIds]).catch(() => ({ rowCount: 0 }));
        incidentsDeleted = r4.rowCount || 0;
      }
      const r5 = await db.raw(`
        DELETE FROM persons
        WHERE phone = ANY(?::text[]) OR full_name = ANY(?::text[])
      `, [SEED_PHONE_PATTERNS, SEED_NAMES]).catch(() => ({ rowCount: 0 }));
