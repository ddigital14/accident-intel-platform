...
const TOOLS = {
  brave_search: { fn: tool_brave_search, params: { query: 'string' },
    description: 'General Brave web search. Returns title+url+snippet for top 8-10 hits across web/news/discussions. Best for broad name searches.' },
  google_cse: { fn: tool_google_cse, params: { query: 'string' },
    description: 'Google Custom Search Engine — different index than Brave, sometimes finds what Brave misses. Lower daily quota; use sparingly.' },
  fetch_url: { fn: tool_fetch_url, params: { url: 'string' },
    description: 'Fetch any URL and return cleaned plain text body (max 6KB). Use this AFTER a search tool returns a promising URL — read the actual article/obituary/profile.' },
  ...
  pdl_identify: { fn: tool_pdl_identify, params: { name: 'string', city: 'string?', state: 'string?' },
    description: 'PeopleDataLabs /identify. Returns ranked matches with employer, email, phone, address, age. Great for U.S. adults with locality.' },
  search_legacy_com: { fn: tool_search_legacy, params: { name: 'string', state: 'string?' },
    description: 'Search Legacy.com (largest US obituary aggregator) via Brave site: query.' },
  search_findagrave: { fn: tool_search_findagrave, params: { name: 'string', state: 'string?' },
    description: 'Search FindAGrave (burial registry) via Brave site: query. Useful for confirming death + sometimes lists family.' },
  search_gofundme: { fn: tool_search_gofundme, params: { name: 'string', state: 'string?' },
    description: 'Search GoFundMe via Brave site:. Organizer is often a family member with a city — high-value next-of-kin lead.' },
  search_caringbridge: { fn: tool_search_caringbridge, params: { name: 'string', state: 'string?' },
    description: 'Search Caringbridge for hospitalized victims. Pages list family + recovery updates.' },
  search_obituary_aggregators: { fn: tool_search_obituary_aggregators, params: { name: 'string', state: 'string?' },
    description: 'Search across Tribute Archive, Dignity Memorial, Ever Loved, FuneralHomes.com via Brave site:.' },
  search_local_news: { fn: tool_search_local_news, params: { name: 'string', city: 'string?', state: 'string?' },
    description: 'General Brave search biased toward local news with the name + city + state.' },
  courtlistener_by_name: { fn: tool_courtlistener_by_name, params: { name: 'string' },
    description: 'CourtListener federal+state PI civil case search. Returns case captions where the name appears.' },
  write_field: { fn: tool_write_field, params: { field: 'string', value: 'any', evidence_url: 'string?' },
    description: 'PERSIST a discovered field (phone, email, address, city, state, age, employer, occupation, attorney_firm, attorney_name, insurance_company, linkedin_url) to the person row in the database. ONLY call this when you have HIGH-CONFIDENCE evidence from a fetched page or search result; pass evidence_url.' },
  add_family_member: { fn: tool_add_family_member, params: { name: 'string', relationship: 'string?', city: 'string?', state: 'string?' },
    description: 'Create a related-person row for a confirmed family member (spouse/parent/child/sibling) found via obit/news. They become research targets in their own right and trigger cascade enrichment.' },
  legacy_direct_search: { fn: tool_legacy_direct, params: { name: 'string', state: 'string?' },
    description: 'Hit Legacy.com search page directly (skipping Brave) — returns the obituary search results HTML.' },
  ...
  truepeoplesearch: { fn: tool_truepeoplesearch, params: { name: 'string', city: 'string?', state: 'string?' },
    description: 'TruePeopleSearch direct fetch — phone+address+relatives for living US adults. Most effective with name+state.' },
  local_news_search: { fn: tool_local_news_search, params: { name: 'string', city: 'string', state: 'string?' },
    description: 'PRIORITIZE THIS TOOL when city is known. Direct fetch of local newspaper search pages across 30 metros (68 papers): IndyStar, NY Post, LA Times, Chicago Tribune, Houston Chronicle, AJC, Miami Herald, Tampa Bay Times, Orlando Sentinel, AZ Central, LV Review-Journal, Seattle Times, Boston Globe, Inquirer, Detroit Free Press, Denver Post, Dallas News, NJ.com, Cleveland.com, Akron Beacon Journal, Cincinnati Enquirer, STL Today, Pittsburgh Post-Gazette, Charlotte Observer, Tennessean, Star Tribune, Oregonian, SF Chronicle, San Diego UT, Sac Bee, etc. Bypasses Brave free-tier sparse small-market indexing. Returns top articles with full body text. Best chance of finding local accident coverage with named victim or family.'
};
