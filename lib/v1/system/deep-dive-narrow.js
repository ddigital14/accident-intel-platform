...
async function apolloByName(name, locality, APOLLO_KEY) {
...
async function pdlByName(name, locality, PDL_KEY) {
...
async function processOne(db, person) {
  ...
  const [apollo, pdl, brave, gcse, voter] = await Promise.all([
    apolloByName(person.full_name, locality, keys.apollo),
    pdlByName(person.full_name, locality, keys.pdl),
