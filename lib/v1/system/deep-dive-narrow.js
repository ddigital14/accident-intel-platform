...
async function apolloByName(name, locality, state, APOLLO_KEY) {
  ...
  const body = { first_name: first, last_name: last, reveal_personal_emails: true };
...
async function pdlByName(name, locality, state, PDL_KEY) {
  // Mirror lib/v1/enrich/pdl-identify.js — GET /v5/person/identify with URL params.
  // /identify accepts partial inputs and ranks matches by likelihood, unlike /search
  // which requires exact ES query and 404s on weak matches.
  ...
  try {
    ...
    if (!r.ok) return null;
    const j = await r.json();
    if (j.status !== 200 || !Array.isArray(j.matches)) return [];
    return j.matches.slice(0, 5).map(m => {
      const p = m.data || {};
      return {
        source: 'pdl',
        name: p.full_name,
        ...
        email: (p.personal_emails?.[0] || p.work_email) || null,
        phone: p.mobile_phone || p.phone_numbers?.[0] || null,
        address: p.location_street_address || null,
        zip: p.location_postal_code || null,
        linkedin: p.linkedin_url,
        age: p.birth_year ? new Date().getFullYear() - p.birth_year : null,
        likelihood: m.match_score
      };
    });
  } catch { return null; }
}
...
async function braveSearchObituary(name, city, state, BRAVE_KEY) {
  ...
  // Three queries fire in parallel: name only, name+state, name+accident
  const q = `"${name}" ${state || ''}`.trim();
  try {
...
async function applyTopCandidate(db, person, top) {
  ...
  const minConf = hasContact ? 0.18 : 0.25;
  ...
  // Apollo / PDL also return employer + linkedin + age
  const employerCand = (top.sample || []).find(s => s.employer)?.employer;
  const linkedinCand = (top.sample || []).find(s => s.linkedin)?.linkedin;
  const ageCand = (top.sample || []).find(s => s.age)?.age;
  if (!person.employer && employerCand) updates.employer = employerCand;
  if (!person.linkedin_url && linkedinCand) updates.linkedin_url = linkedinCand;
  if (!person.age && ageCand) updates.age = ageCand;
