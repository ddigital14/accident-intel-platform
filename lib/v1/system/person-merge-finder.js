...
// Phase 66c: Levenshtein distance for name-mismatch reject
...
async function scan(db, opts = {}) {
  ...
  for (let i = 0; i < people.length; i++) {
    ...
    for (const cand of top) {
      ...
      // Phase 66c: name-similarity gate — reject if embedding paired persons whose names are far apart
      // (defends against summary-string overlap on city='Unknown,GA' etc.)
      if (!namesAreClose(A.full_name, B.full_name)) continue;
...
async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'scan') {
      ...
      // Phase 67 bonus: admin one-click — confirm merges where similarity = 1.000 AND name match exact
  if (action === 'confirm_all_perfect_merges') {
    await ensureProposalTable(db);
    const props = await db('person_merge_proposals')
      .where('status', 'proposed')
      .where('similarity_score', '>=', 0.999)
      .select('id', 'person_a_id', 'person_b_id');
    let confirmed = 0;
    for (const pp of props) {
      const persons = await db('persons').whereIn('id', [pp.person_a_id, pp.person_b_id]).select('id', 'full_name');
      if (persons.length < 2) continue;
      const n0 = String(persons[0].full_name || '').trim().toLowerCase();
      const n1 = String(persons[1].full_name || '').trim().toLowerCase();
      if (n0 !== n1) continue;  // require exact name match for auto-confirm
      try {
        await db('person_merge_proposals').where('id', pp.id).update({
          status: 'confirmed', reviewed_at: new Date(), reviewed_by: 'admin:perfect_match_auto'
        });
        confirmed++;
      } catch (_) {}
    }
    return res.json({ success: true, confirmed_count: confirmed });
  }

  if (action === 'reject_low_quality') {
    await ensureProposalTable(db);
    const props = await db('person_merge_proposals').where('status', 'proposed').select('id', 'person_a_id', 'person_b_id');
    let rejected = 0;
    for (const pp of props) {
      const persons = await db('persons').whereIn('id', [pp.person_a_id, pp.person_b_id]).select('id', 'full_name');
      if (persons.length < 2) continue;
      if (!namesAreClose(persons[0].full_name, persons[1].full_name)) {
        await db('person_merge_proposals').where('id', pp.id).update({ status: 'rejected', reviewed_at: new Date(), reviewed_by: 'auto:name_too_far' });
        rejected++;
      }
    }
    return res.json({ success: true, rejected_count: rejected });
  }
