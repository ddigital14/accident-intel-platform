...
async function scan(db, opts = {}) {
  ...
  try {
    people = await db('persons')
      .select('id', 'full_name', 'city', 'state', 'age', 'employer', 'role', 'updated_at')
  ...
  for (let i = 0; i < people.length; i++) {
    ...
    for (const cand of top) {
      ...
      if (!rolesCompatible(A.role, B.role)) continue;
...
async function listProposals(db, status = 'proposed', limit = 100) {
  ...
  const persons = ids.size === 0 ? [] : await db('persons')
    .select('id', 'full_name', 'city', 'state', 'age', 'employer', 'role', 'phone', 'email', 'updated_at')
