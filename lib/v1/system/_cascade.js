...
// PHASE 40: action-routing dispatcher.  When trigger_source === 'auto_resolve' (or
// any other action-key in ACTION_HANDLERS), the queue processor calls the mapped
// handler instead of the default runCascadeForPerson.  Lets us fire
// victim-contact-finder synchronously for newly-verified victims.
const ACTION_HANDLERS = {
  // Newly-verified victim — kick off composite contact resolver immediately
  auto_resolve: async function(db, job) {
    if (!job.person_id) return null;
    try {
      const finder = require('../enrich/victim-contact-finder');
      let result = null;
      if (typeof finder.resolveOne === 'function') {
        const person = await db('persons').where('id', job.person_id).first();
        if (!person) return { error: 'person_not_found' };
        result = await finder.resolveOne(db, person);
      } else {
        const fn = typeof finder === 'function' ? finder : (finder.handler || finder.default);
        if (!fn) return { error: 'finder_unavailable' };
        const fakeReq = { method: 'GET', query: { secret: 'ingest-now', action: 'resolve', person_id: job.person_id }, headers: {}, body: null, url: '' };
        let body = null;
      ...
      // Phase 44A: also queue realtime_alert so Slack/SMS fire <60s after verification
      try {
        await enqueueCascade(db, {
          person_id: job.person_id,
          incident_id: job.incident_id,
          trigger_source: 'realtime_alert',
          trigger_field: 'victim_verified',
          trigger_value: 'true',
          priority: 8
        });
      } catch (_) {}
      return { action: 'auto_resolve', result, queued_realtime_alert: true };
    } catch (e) {
      return { action: 'auto_resolve', error: e.message };
    }
  },
  // Phase 44A: dispatch realtime alert for a freshly-verified victim
  realtime_alert: async function(db, job) {
    if (!job.person_id) return null;
    try {
      const rta = require('./realtime-victim-alerts');
      const person = await db('persons').where('id', job.person_id).first();
      if (!person) return { error: 'person_not_found' };
      if (job.incident_id) {
        const inc = await db('incidents').where('id', job.incident_id).first();
        if (inc) {
      ...
      const r = await rta.alertOne(db, person);
      return { action: 'realtime_alert', result: r };
    } catch (e) {
      return { action: 'realtime_alert', error: e.message };
    }
  },
  // Fatal-victim → pull family from obituary survivors, then queue them too
  fatal_family: async function(db, job) {
    if (!job.person_id) return null;
    try {
      const survivors = require('../enrich/funeral-home-survivors');
      const person = await db('persons').where('id', job.person_id).first();
      if (!person) return { error: 'person_not_found' };
      if (typeof survivors.resolveOne === 'function') {
        const r = await survivors.resolveOne(db, person);
        return { action: 'fatal_family', result: r };
      }
      return { error: 'survivors_unavailable' };
    } catch (e) {
      return { action: 'fatal_family', error: e.message };
    }
  },
  // PHASE 55 (2026-04-30): Auto Fan-Out — fires every relevant engine on persons UPDATE/INSERT
  auto_fan_out: async function(db, job) {
    if (!job.person_id) return null;
    try {
      const fanOut = require('./auto-fan-out');
      const r = await fanOut.runFanOut(db, job.person_id, {
        trigger_field: job.trigger_field,
        contactFieldChanged: !!job.contact_field_changed,
        force: false
      });
      return { action: 'auto_fan_out', engines_fired: r.engines_fired, ok_count: r.ok_count, filled: r.filled_in_this_pass };
    } catch (e) {
      return { action: 'auto_fan_out', error: e.message };
    }
  }
};

function _resolveActionHandler(job) {
  const key = String(job.trigger_source || '').toLowerCase();
  return ACTION_HANDLERS[key] || null;
}
...
async function processCascadeQueue(db, opts = {}) {
  ...
  for (const job of pending) {
    ...
    try {
      ...
      let log = null;
      const actionFn = _resolveActionHandler(job);
      if (actionFn) {
        const r = await actionFn(db, job);
        log = { action_routed: job.trigger_source, result: r, total_fields_filled: 0, confidence_before: 0, confidence_after: 0, sources_fired: [job.trigger_source] };
      } else if (job.person_id) {
...
/**
 * Phase 21 Wire #9: Incident-level cascade.
 * When an incident gains has_attorney_known=true OR severity changes OR
 * lead_score jumps significantly, re-run cross-conversion across ALL its
 * persons (re-cross-exam + re-claude-reason). Persons cascades alone don't
 * pick up incident-only signal changes.
 */
async function enqueueIncidentCascade(db, incidentId, opts = {}) {
  await ensureQueueTable(db);
  const { trigger_source = 'incident_change', trigger_field, trigger_value, priority = 5 } = opts;
  // Look up persons on this incident; enqueue each one with the trigger
  const persons = await db('persons').where('incident_id', incidentId).select('id');
  const enqueued = [];
  for (const p of persons) {
    const r = await enqueueCascade(db, {
      person_id: p.id,
    ...
    if (r?.enqueued) enqueued.push(r.queue_id);
  }
  // Also queue a synthetic incident-only entry (no person_id) so claude reasoner picks it up
  const [row] = await db('cascade_queue').insert({
    incident_id: incidentId,
    trigger_source,
    trigger_field: trigger_field ? String(trigger_field).substring(0, 80) : null,
  ...
  return { enqueued: enqueued.length, queue_ids: enqueued, incident_queue_id: row?.id };
}

module.exports = {
  enqueueCascade,
  enqueueIncidentCascade,
  processCascadeQueue,
  runCascadeForPerson,
  ensureQueueTable,
  ACTION_HANDLERS
