  ...
  auto_resolve: async function(db, job) {
    ...
    try {
      ...
      if (typeof finder.resolveOne === 'function') {
        ...
        if (!person) return { error: 'person_not_found' };
        const r = await finder.resolveOne(db, person);
        return { action: 'auto_resolve', result: r };
      }
      // Fall-through if finder API ever changes — invoke as HTTP-ish handler
      ...
      return { action: 'auto_resolve', result: body };
    } catch (e) {
