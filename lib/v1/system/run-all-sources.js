...
async function findOrCreatePerson(db, params) {
  ...
  // Try exact match by name + state, then loosen
  const stateUC = state ? String(state).toUpperCase() : null;
  let p = await db('persons')
    .whereRaw('LOWER(full_name) = LOWER(?)', [victim_name])
    .modify(q => stateUC && q.where('state', stateUC))
    .first();
  if (!p) {
    p = await db('persons').whereRaw('LOWER(full_name) = LOWER(?)', [victim_name]).first();
  }
  if (!p && victim_name.includes(' ')) {
    const parts = String(victim_name).trim().split(/\s+/);
    const first = parts[0]; const last = parts[parts.length - 1];
    p = await db('persons')
      .whereRaw('LOWER(first_name) = LOWER(?) AND LOWER(last_name) = LOWER(?)', [first, last])
      .modify(q => stateUC && q.where('state', stateUC))
      .first();
  }
  if (p) return { person: p, created: false };

  // No existing person AND no incident_id can be inferred — return a transient
  // in-memory person object so we can still fire the public-records sources that
  // only need name/city/state.  We intentionally do NOT insert: persons.incident_id
  // is NOT NULL in this schema, and creating an orphan would break referential rules.
  const parts2 = String(victim_name).trim().split(/\s+/);
  return {
    person: {
      id: null,
      full_name: victim_name,
      first_name: parts2[0] || null,
      last_name: parts2.length > 1 ? parts2[parts2.length - 1] : null,
      city: city || null,
      state: stateUC,
...
async function runAll(db, person, opts = {}) {
  ...
  if (!personId) {
    // Transient person (no DB row) — we can only run name-only public record
    // searches via the rep-quick-links generator.  Build links and bail.
    const { buildLinks } = require('../dashboard/rep-quick-links');
    const links = buildLinks(person);
    return {
      person_id: null,
      transient: true,
      person: {
        full_name: person.full_name, city: person.city, state: person.state,
      ...
      sources_ok: 0,
      steps: [{
        step: 'transient_no_db_row',
        ok: true,
        note: 'No matching DB person + persons.incident_id is NOT NULL; returning rep-quick-links instead. POST with ?person_id=<existing-victim-uuid> to run the full pipeline.',
