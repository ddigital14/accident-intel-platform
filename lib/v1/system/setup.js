  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (body.apollo_api_key) {
        await setConfig(db, 'apollo_api_key', body.apollo_api_key);
        updates.apollo_api_key = 'set';
      }
      if (body.deepgram_api_key) {
        await setConfig(db, 'deepgram_api_key', body.deepgram_api_key);
        updates.deepgram_api_key = 'set';
      }
      if (body.voyageai_api_key) {
        await setConfig(db, 'voyageai_api_key', body.voyageai_api_key);
        updates.voyageai_api_key = 'set';
      }
      if (body.brave_api_key) {
        await setConfig(db, 'brave_api_key', body.brave_api_key);
        updates.brave_api_key = 'set';
      }
      if (body.brave_answers_api_key) {
        await setConfig(db, 'brave_answers_api_key', body.brave_answers_api_key);
        updates.brave_answers_api_key = 'set';
      }
      if (body.courtlistener_api_token) {
        await setConfig(db, 'courtlistener_api_token', body.courtlistener_api_token);
        updates.courtlistener_api_token = 'set';
      }
      if (body.resend_api_key) {
        await setConfig(db, 'resend_api_key', body.resend_api_key);
        updates.resend_api_key = 'set';
      }
      if (body.resend_from_email) {
        await setConfig(db, 'resend_from_email', body.resend_from_email);
        updates.resend_from_email = 'set';
      }
      if (body.newsapi_key) {
      ...
      if (Array.isArray(body.create_reps)) {
        ...
        for (const r of body.create_reps) {
          ...
          if (exists) {
            ...
            // Phase 57: allow password reset on existing user when r.password or r.reset_password provided
            let pwReset = null;
            if (r.password) {
              upd.password_hash = await bcrypt.hash(r.password, 10);
              pwReset = r.password;
            } else if (r.reset_password === true) {
              const fresh = `${(r.first_name || exists.first_name || 'rep').toLowerCase()}${Date.now() % 100000}!`;
              upd.password_hash = await bcrypt.hash(fresh, 10);
              pwReset = fresh;
            }
            await db('users').where('id', exists.id).update(upd);
            const out = { id: exists.id, action: 'updated' };
            if (pwReset) out.temp_password = pwReset;
            created.push(out);
          } else {
