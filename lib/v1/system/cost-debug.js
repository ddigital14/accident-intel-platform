...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Run the same SUM queries the cost endpoint runs
    try {
      const day24 = new Date(Date.now() - 86400000);
      const sumDay = await db.raw('SELECT SUM(cost_usd) as total, COUNT(*) as c FROM system_api_calls WHERE created_at > ?', [day24]);
      out.sum_24h = sumDay.rows[0];
      const groupSvc = await db.raw('SELECT service, COUNT(*) as calls, SUM(cost_usd) as cost FROM system_api_calls WHERE created_at > ? GROUP BY service ORDER BY cost DESC LIMIT 10', [day24]);
      out.by_service = groupSvc.rows;
    } catch (e) {
      out.sum_error = e.message;
    }
