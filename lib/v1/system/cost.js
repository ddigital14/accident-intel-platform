...
const PRICING = {
  ...
  'trestle':                    { flat: 0.05 },  // generic fallback
  'trestle__3_2_phone':          { flat: 0.07 },
  'trestle__3_1_cnam':           { flat: 0.05 },
  'trestle__3_2_location':       { flat: 0.07 },
  'trestle__3_2_caller_id':      { flat: 0.07 },
  'trestle__3_0_real_contact':   { flat: 0.03 },
  'trestle__3_0_phone_intel':    { flat: 0.015 },
...
async function trackApiCall(db, pipeline, service, tokensIn = 0, tokensOut = 0, success = true) {
  try {
    ...
    console.error('trackApiCall failed:', e.message);
  }
}
  ...
  try {
    ...
    const [byService24h, byPipeline24h, totals] = await Promise.all([
      db.raw(`SELECT service, COUNT(*) as calls, SUM(tokens_in) as tin, SUM(tokens_out) as tout, SUM(cost_usd) as cost
              FROM system_api_calls WHERE created_at > $1 GROUP BY service ORDER BY cost DESC`, [day])
        .then(r => r.rows || []).catch(() => []),
      db.raw(`SELECT pipeline, COUNT(*) as calls, SUM(cost_usd) as cost
              FROM system_api_calls WHERE created_at > $1 GROUP BY pipeline ORDER BY cost DESC`, [day])
        .then(r => r.rows || []).catch(() => []),
      Promise.all([
        db.raw(`SELECT SUM(cost_usd) as total FROM system_api_calls WHERE created_at > $1`, [day])
          .then(r => parseFloat(r.rows?.[0]?.total || 0)).catch(() => 0),
        db.raw(`SELECT SUM(cost_usd) as total FROM system_api_calls WHERE created_at > $1`, [week])
          .then(r => parseFloat(r.rows?.[0]?.total || 0)).catch(() => 0),
        db.raw(`SELECT SUM(cost_usd) as total FROM system_api_calls WHERE created_at > $1`, [month])
          .then(r => parseFloat(r.rows?.[0]?.total || 0)).catch(() => 0)
      ])
    ]);
