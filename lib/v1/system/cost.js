...
async function trackApiCall(db, pipeline, service, tokensIn = 0, tokensOut = 0, success = true) {
  ...
  try {
    ...
    // Phase 50: piggyback CEI invocation telemetry — every successful API call
    // bumps engine_capabilities.invocation_count for its pipeline.
    try {
      const { bumpCounter } = require('./_cei_telemetry');
      bumpCounter(db, pipeline, !!success, 0).catch(() => {});
    } catch (_) {}
  } catch (e) {
