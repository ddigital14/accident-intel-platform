...
// Phase 44B: route via existing AI router — handles model fallback (Opus→Sonnet)
let _aiRouter = null;
function getAiRouter() {
  if (_aiRouter) return _aiRouter;
  try { _aiRouter = require('../enrich/_ai_router'); } catch (_) { _aiRouter = null; }
  return _aiRouter;
}
...
const ENGINE_CATALOG = [
  ...
  { id: 'deep_phone_research',   route: 'enrich/deep-phone-research',   provider: 'anthropic',cost: 0.20, fills: ['phone','email','address','family'],gates: ['has_name'], depth: 'deepest' }
];
...
async function aiReason(db, baseline) {
  if (!baseline.actions?.length) return baseline;
  const router = getAiRouter();
  if (!router || typeof router.extract !== 'function') return { ...baseline, ai_skipped: true, ai_error: 'no_ai_router' };
  const userPrompt = `You are the Cross-Intelligence Orchestrator for an accident-intel platform. A verified victim is below with current context, missing fields, and candidate next-engine actions. Pick the top 3 actions in order. Re-rank using:
...
${JSON.stringify(baseline, null, 2)}`;
  try {
    const r = await router.extract(db, {
      pipeline: 'cross-intel-orchestrator',
      tier: 'opus',
      provider: 'claude',
      systemPrompt: 'You are a senior cross-intelligence orchestrator. Always return valid JSON only.',
      userPrompt,
      responseFormat: 'json',
      timeoutMs: HTTP_TIMEOUT_MS
    });
    if (!r?.ok) {
      const top = (baseline.actions || []).slice(0, 3).map(a => a.id).join(', ');
      return {
        ...baseline,
        ai_skipped: true,
        ai_error: r?.error || 'router_fail',
        plan_summary: `Heuristic plan (AI unavailable): try ${top}.`
      };
    }
    const parsed = r.parsed || (() => { try { const m = String(r.raw || '').match(/\{[\s\S]*\}/); return m ? JSON.parse(m[0]) : null; } catch (_) { return null; } })();
    if (!parsed) {
      const top = (baseline.actions || []).slice(0, 3).map(a => a.id).join(', ');
      return {
        ...baseline,
        ai_skipped: true,
        ai_raw: String(r.raw || '').slice(0, 500),
        plan_summary: `Heuristic plan (AI unparsable): try ${top}.`
      };
    }
    ...
    return { ...baseline, actions: reordered, plan_summary: parsed.plan_summary || null, ai_used: true, model_used: r.model_used };
  } catch (e) {
    const top = (baseline.actions || []).slice(0, 3).map(a => a.id).join(', ');
    return {
      ...baseline,
      ai_skipped: true,
      ai_error: e.message,
      plan_summary: `Heuristic plan (AI exception): try ${top}.`
    };
  }
}
