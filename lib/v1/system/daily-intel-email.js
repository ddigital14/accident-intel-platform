  ...
  <table cellpadding="0" cellspacing="0" border="0" style="width:100%;border-collapse:collapse;font-size:11px;background:#FFFFFF;">
    <thead><tr style="background:#F1F5F9;">
      ...
      ${(() => {
        ...
        // Sort each tier by lead_score DESC
        ...
        const renderRow = p => {
          ...
          const noteHtml = (() => {
            try {
              if (!p.discrepancy_log) return '';
              const parsed = typeof p.discrepancy_log === 'string' ? JSON.parse(p.discrepancy_log) : p.discrepancy_log;
              if (parsed?.note) {
                const sevColor = parsed.severity === 'high' ? '#B91C1C' : (parsed.severity === 'medium' ? '#92400E' : '#166534');
                return `<br/><span style="color:${sevColor};font-style:italic;font-size:9px;display:block;margin-top:3px;max-width:260px;">⚠ ${parsed.note.slice(0, 180)}</span>`;
              }
            } catch(_) {}
            return '';
          })();
          return `<tr style="border-bottom:1px solid #F1F5F9;">
            ...
            <td style="padding:6px 10px;color:#0F172A;font-family:JetBrains Mono,Menlo,monospace;font-size:10px;">${p.phone || '—'}</td>
            <td style="padding:6px 10px;color:#0F172A;font-family:JetBrains Mono,Menlo,monospace;font-size:10px;">${(p.email || '—').slice(0, 30)}</td>
            ...
            <td style="padding:6px 10px;text-align:right;font-weight:700;color:${p.lead_score >= 80 ? '#EF4444' : '#1F2937'};">${p.lead_score || 0}</td>
          </tr>`;
        };

        const sections = [];
        if (tiers.normal.length) {
          sections.push(`<tr><td colspan="6" style="background:#ECFDF5;padding:10px 12px;font-weight:700;color:#047857;font-size:12px;text-transform:uppercase;letter-spacing:0.05em;">✓ Normal Leads (${tiers.normal.length})</td></tr>` +
            tiers.normal.slice(0, 30).map(renderRow).join(''));
        }
        if (tiers.review.length) {
          sections.push(`<tr><td colspan="6" style="background:#FFFBEB;padding:10px 12px;font-weight:700;color:#92400E;font-size:12px;text-transform:uppercase;letter-spacing:0.05em;">⚠ Needs Review (${tiers.review.length}) — discrepancy noted, manual research recommended</td></tr>` +
            tiers.review.slice(0, 20).map(renderRow).join(''));
        }
        if (tiers.demoted.length) {
          sections.push(`<tr><td colspan="6" style="background:#FEF2F2;padding:10px 12px;font-weight:700;color:#B91C1C;font-size:12px;text-transform:uppercase;letter-spacing:0.05em;">✗ Demoted (${tiers.demoted.length}) — major inconsistency, hidden from active queue</td></tr>` +
            tiers.demoted.slice(0, 10).map(renderRow).join(''));
        }
        return sections.join('');
      })()}
      ${false ? `${contactablePersons.slice(0, 50).map(p => {
        const verified = p.victim_verified ? '<span style="color:#047857;font-weight:600;">✓ verified</span>' : '<span style="color:#64748B;">unverified</span>';
        const qstate = p.qualification_state === 'qualified' ? '<span style="color:#EF4444;font-weight:700;">QUALIFIED</span>' : (p.qualification_state || '');
        return `<tr style="border-bottom:1px solid #F1F5F9;">
          <td style="padding:6px 10px;font-weight:600;color:#1F2937;">${(p.full_name || '?').slice(0, 30)}</td>
          ...
          <td style="padding:6px 10px;color:#0F172A;font-family:JetBrains Mono,Menlo,monospace;font-size:10px;">${(p.email || '—').slice(0, 30)}</td>
          <td style="padding:6px 10px;color:#475569;">${(p.city || '?')}, ${(p.state || '?')}</td>
          <td style="padding:6px 10px;font-size:10px;">${verified}<br/>${qstate}${(() => {
            try {
              if (!p.discrepancy_log) return '';
              const parsed = typeof p.discrepancy_log === 'string' ? JSON.parse(p.discrepancy_log) : p.discrepancy_log;
              if (parsed?.note) return `<br/><span style="color:#92400E;font-style:italic;font-size:9px;display:block;margin-top:3px;max-width:200px;">⚠ ${parsed.note.slice(0, 120)}</span>`;
            } catch(_) {}
            return '';
          })()}</td>
          <td style="padding:6px 10px;text-align:right;font-weight:700;color:${p.lead_score >= 80 ? '#EF4444' : '#1F2937'};">${p.lead_score || 0}</td>
  ...
  try {
    contactablePersons = await db('persons')
      ...
      .select('persons.id', 'persons.full_name', 'persons.phone', 'persons.email', 'persons.address', 'persons.city', 'persons.state',
              'persons.attorney_firm', 'persons.victim_verified',
              db.raw('persons.lead_tier as lead_tier'),
              'incidents.qualification_state', 'incidents.lead_score', 'incidents.severity',
