    ...
    <thead><tr style="background:#F1F5F9;">
      ...
      ${(() => {
        ...
        const renderRow = (p) => {
          ...
          let scriptHtml = '';
          try {
            if (p.call_script_log) {
              const cs = typeof p.call_script_log === 'string' ? JSON.parse(p.call_script_log) : p.call_script_log;
              if (cs && typeof cs === 'object' && cs.primary_line) {
                const line = String(cs.primary_line).slice(0, 200) + (String(cs.primary_line).length > 200 ? '…' : '');
                scriptHtml = '<div style="margin-top:4px;padding:6px 9px;background:#ECFDF5;border-left:3px solid #10B981;border-radius:4px;font-size:10px;color:#065F46;line-height:1.4;"><strong style="color:#047857;">📞 Suggested opening line:</strong> <span style="font-style:italic;">' + line.replace(/</g,'&lt;') + '</span></div>';
              }
            }
          } catch(_) {}
          return '<tr style="border-bottom:1px solid #F1F5F9;">' +
            ...
            '<td style="padding:6px 10px;font-size:10px;">' + verified + '<br/>' + qstate + noteHtml + scriptHtml + '</td>' +
  ...
  try {
    contactablePersons = await db('persons')
      ...
      .select('persons.id', 'persons.full_name', 'persons.phone', 'persons.email', 'persons.address', 'persons.city', 'persons.state',
              ...
              db.raw("(SELECT new_value FROM enrichment_logs WHERE person_id = persons.id AND field_name = 'rep_call_script' ORDER BY created_at DESC LIMIT 1) as call_script_log"))
