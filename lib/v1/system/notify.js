  ...
  try {
    ...
    for (const inc of candidates) {
      try {
        ...
        if (smsTo) {
          // TCPA litigator pre-check — refuse SMS if blocked
          const tcpaCheck = await checkLitigator(db, smsTo).catch(() => ({ blocked: false }));
          if (tcpaCheck && tcpaCheck.blocked) {
            results.skipped++;
            results.errors.push(`${inc.id}: SMS blocked by TCPA guard (risk=${tcpaCheck.risk_score}, sources=${(tcpaCheck.sources || []).join(',')})`);
          } else {
