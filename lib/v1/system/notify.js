...
const { checkLitigator } = require('../enrich/tcpa-litigator-check');
...
async function preFlightLookup(db, to) {
  // Pre-flight Twilio Lookup: confirms number exists + grabs carrier/caller_name (free identity confidence)
  if (!TWILIO_SID || !TWILIO_TOKEN || !to) return null;
  try {
    const auth = Buffer.from(`${TWILIO_SID}:${TWILIO_TOKEN}`).toString('base64');
    const url = `https://lookups.twilio.com/v2/PhoneNumbers/${encodeURIComponent(to)}?Fields=line_type_intelligence`;
    const resp = await fetch(url, { headers: { Authorization: `Basic ${auth}` }, signal: AbortSignal.timeout(6000) });
    if (!resp.ok) return null;
    const data = await resp.json();
    return { valid: data.valid !== false, line_type: data.line_type_intelligence?.type, carrier: data.line_type_intelligence?.carrier_name };
  } catch (_) { return null; }
}
  ...
  try {
    ...
    for (const inc of candidates) {
      try {
        ...
        // Phase 48: BROADCAST SMS to ALL active reps + admins with phone numbers
        // (Fritzi Housley + Mason Donovan + any others). If incident has assigned rep, prioritize them.
        const recipients = new Set();
        if (inc.assigned_to) {
          const rep = await db('users').where('id', inc.assigned_to).first();
          if (rep?.phone) recipients.add(rep.phone);
        }
        // Always include all active reps + admins (multi-recipient broadcast)
        const allActiveStaff = await db('users')
          .whereIn('role', ['rep', 'admin', 'manager'])
          .where('is_active', true)
          .whereNotNull('phone')
          .select('phone', 'first_name');
        for (const u of allActiveStaff) {
          if (u.phone) recipients.add(u.phone);
        }
        // Fallback if no active reps configured
        if (!recipients.size && ALERT_SMS_TO) recipients.add(ALERT_SMS_TO);

        const msg = `AIP: New ${inc.severity || ''} ${inc.incident_type} in ${inc.city || '?'} ${inc.state || ''}. Score ${inc.lead_score}. ${persons[0]?.full_name || ''} ${persons[0]?.phone || persons[0]?.email || ''}`.trim();

        for (const smsTo of recipients) {
          // TCPA litigator pre-check per recipient
          const tcpaCheck = await checkLitigator(db, smsTo).catch(() => ({ blocked: false }));
          if (tcpaCheck && tcpaCheck.blocked) {
            results.skipped++;
            results.errors.push(`${inc.id}: SMS to ${smsTo} blocked by TCPA guard (risk=${tcpaCheck.risk_score})`);
            continue;
          }
          const smsOK = await sendSMS(smsTo, msg);
