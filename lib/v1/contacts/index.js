...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      // Combined contact filter — at least one of phone/email/address
      const has_any_contact = req.query.has_any_contact;
      if (has_any_contact === 'true') {
        query = query.where(function() {
          this.where(function() { this.whereNotNull('p.phone').andWhere('p.phone', '!=', ''); })
