...
const QUERIES = [
  '"just witnessed" crash accident',
  '"praying for" car accident crash family',
  '"identified as" crash victim',
  '"struck and killed" pedestrian',
  '"hit and run" died killed',
  '"GoFundMe" car accident',
  '"police identified" crash victim',
  'fatal crash named family',
  '"loved ones" "car accident" tribute'
];

async function searchBrave(key, q) {
  try {
    const r = await fetch(`https://api.search.brave.com/res/v1/web/search?q=${encodeURIComponent(q)}&count=15`, {
      headers: { 'Accept': 'application/json', 'X-Subscription-Token': key },
      signal: AbortSignal.timeout(10000)
    });
    if (!r.ok) return [];
    const j = await r.json();
    // Filter to social-media or news domains, where witness/family posts live
    const SOCIAL_RE = /(twitter\.com|x\.com|facebook\.com|instagram\.com|reddit\.com|tiktok\.com|nextdoor\.com)/i;
    const NEWS_RE = /(news|tribune|herald|chronicle|gazette|times|post|patch\.com|abc\d|nbc\d|cbs\d|fox\d|cnn\.com)/i;
    return (j.web?.results || [])
      .filter(x => SOCIAL_RE.test(x.url || '') || NEWS_RE.test(x.url || ''))
      .map(x => ({ title: x.title, url: x.url, snippet: x.description, age: x.age }));
  } catch { return []; }
}
