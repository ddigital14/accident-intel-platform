...
const FEEDS = [
  ...
  { url: 'https://www.miamiherald.com/news/local/?widgetName=rssfeed&widgetContentId=712015&getXmlFeed=true', metro: 'Miami', name: 'Miami Herald' },
  { url: 'https://www.wftv.com/feed/rss/news', metro: 'Orlando', name: 'WFTV Orlando' },
...
const CRASH_KEYWORDS = /crash|accident|collision|killed|injured|struck|wreck|rollover|fatal|pedestrian|motorcyclist|hit-and-run|hit and run|trooper|state police|trauma|hospital|life-threatening/i;
...
async function extractWithGPT(article, hintCity) {
  ...
  try {
    const resp = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: 'Extract crash JSON only. is_crash:false if not crash.' },
      ...
      try {
        ...
        for (const it of crashItems.slice(0, 5)) {
          ...
          try {
            ...
            const parsed = await extractWithGPT(it, feed.metro);
