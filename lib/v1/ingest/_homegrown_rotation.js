...
async function ingestHospitalPressReleases(db) {
  ...
  for (const u of FEEDS) {
    ...
    for (const it of items.slice(0, 10)) {
      ...
      if (!/\b(crash|collision|accident|wreck|hit[- ]and[- ]run|rollover|pileup|injured|injury|killed|fatal|dead|hospitalized|trauma|pedestrian|motorcycle|bicycl|truck|semi|tractor[- ]trailer|drowning|electrocut|fall|fell|workplace|osha|forklift|scaffold|dog[- ]bite|elevator|escalator|malpractice|misdiagnos|defective|recall|exploded|paralyzed|amputat|burn|dui|dwi|drunk[- ]driv|wrongful[- ]death|negligen|personal[- ]injur)\b/i.test(text)) continue;
      result.items++;
  ...
  for (const u of URLS) {
    ...
    try {
      ...
      for (const inc of list.slice(0, 30)) {
        result.items++;
        if (!/\b(crash|collision|accident|wreck|hit[- ]and[- ]run|rollover|pileup|injured|injury|killed|fatal|dead|hospitalized|trauma|pedestrian|motorcycle|bicycl|truck|semi|tractor[- ]trailer|drowning|electrocut|fall|fell|workplace|osha|forklift|scaffold|dog[- ]bite|elevator|escalator|malpractice|misdiagnos|defective|recall|exploded|paralyzed|amputat|burn|dui|dwi|drunk[- ]driv|wrongful[- ]death|negligen|personal[- ]injur)\b/i.test(inc.title || inc.raw || '')) continue;
...
async function ingestNextdoorPublic(db) {
  ...
  for (const u of URLS) {
    ...
    if (!/\b(crash|collision|accident|wreck|hit[- ]and[- ]run|rollover|pileup|injured|injury|killed|fatal|dead|hospitalized|trauma|pedestrian|motorcycle|bicycl|truck|semi|tractor[- ]trailer|drowning|electrocut|fall|fell|workplace|osha|forklift|scaffold|dog[- ]bite|elevator|escalator|malpractice|misdiagnos|defective|recall|exploded|paralyzed|amputat|burn|dui|dwi|drunk[- ]driv|wrongful[- ]death|negligen|personal[- ]injur)\b/i.test(plain)) continue;
