...
// Phase 20 #3 — state DOT crash feeds.
//   TX: ArcGIS service backing 511.tx.gov (FREE, no key)
//   GA: 511 GA (requires GA511_KEY env)
//   FL: FL511 events feed v2 (public — flipped on)
//   OH: ODOT OHGO incidents (FREE, no key, public JSON)
//   FL DHSMV crash reports + OH ODPS public-records portal are FOIA-only.
//   See docs/state-foia-paths.md for the request workflow.
const STATES = [
  { code: 'TX', name: 'Texas',
    feedUrl: 'https://services1.arcgis.com/KTcxiTD9dsQw4r7Z/arcgis/rest/services/Active_Crashes/FeatureServer/0/query?where=1%3D1&outFields=*&f=json&resultRecordCount=200',
    parser: 'arcgis_tx', enabled: true },
  { code: 'GA', name: 'Georgia',
    feedUrl: process.env.GA511_KEY
      ? `https://www.511ga.org/api/v2/getevents?key=${process.env.GA511_KEY}&format=json`
      : 'https://www.511ga.org/api/v2/getevents?format=json',
    parser: 'ga511', enabled: !!process.env.GA511_KEY },
  { code: 'FL', name: 'Florida',
    feedUrl: 'https://fl511.com/map/data/Events?eventCategory=Crash',
    parser: 'fl511_v2', enabled: true },
  { code: 'OH', name: 'Ohio',
    feedUrl: 'https://publicapi.ohgo.com/api/v1/incidents?api-key=' + (process.env.OHGO_API_KEY || ''),
    parser: 'ohgo_incidents', enabled: !!process.env.OHGO_API_KEY }
];
...
function parseArcGisTx(json) {
  try {
    return (json.features || []).slice(0, 200).map(f => {
      const a = f.attributes || {};
      const g = f.geometry || {};
      return {
        source_reference: String(a.OBJECTID || a.IncidentID || a.id || ''),
        title: a.Description || a.IncidentType || 'TX crash',
        description: a.Description || '',
        latitude: g.y || a.Latitude,
        longitude: g.x || a.Longitude,
        city: a.City || null,
        state: 'TX',
        occurred_at: a.StartTime ? new Date(a.StartTime) : new Date(),
      };
    }).filter(r => r.latitude && r.longitude);
  } catch (_) { return []; }
}

function parseFl511V2(json) {
  try {
    return (json.events || json.Events || []).slice(0, 200).map(e => ({
      source_reference: String(e.id || e.Id || ''),
      title: e.title || e.Title || 'FL crash',
      description: e.description || e.Description || '',
      latitude: e.point?.lat || e.Latitude,
      longitude: e.point?.lng || e.Longitude,
      city: e.city || e.City || null,
      state: 'FL',
      occurred_at: e.startTime ? new Date(e.startTime) : new Date(),
    })).filter(r => r.latitude && r.longitude);
  } catch (_) { return []; }
}

function parseOhgoIncidents(json) {
  try {
    const arr = json?.results || json?.value || (Array.isArray(json) ? json : []);
    return arr.filter(r => /crash|accident|collision/i.test(r.category || r.eventType || r.description || ''))
      .slice(0, 200).map(r => ({
        source: 'state_ohgo',
        source_reference: `OHGO-${r.id || r.Id || Date.now()}`,
        incident_type: 'car_accident',
        severity: /injury|fatal|serious|major/i.test(r.description || r.severity || '') ? 'serious' : 'moderate',
        title: `${r.category || 'Crash'} - ${r.location || r.roadName || 'Ohio'}`,
        description: `OHGO: ${r.description || ''}. ${r.location || ''}.`,
        state: 'OH',
        city: r.locationName || r.city || null,
        highway: r.roadName || r.route || null,
        lat: parseFloat(r.latitude || r.lat) || null,
        lng: parseFloat(r.longitude || r.lng || r.lon) || null,
        occurred_at: r.startTime || r.reportedAt || new Date().toISOString(),
        confidence: 88, raw: r
      }));
  } catch (_) { return []; }
}

const PARSERS = { txdot: parseTxDOT, ga511: parseGa511, fl511: parseFl511, arcgis_tx: parseArcGisTx, fl511_v2: parseFl511V2, ohgo_incidents: parseOhgoIncidents };
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    for (const st of STATES.filter(s => s.enabled !== false)) {
      try {
