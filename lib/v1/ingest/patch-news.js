...
async function fetchMarket(slug) {
  // Patch killed RSS in 2024. We scrape the market homepage and pull article cards.
  const url = marketToFeedUrl(slug);
  if (!url) return [];
  try {
    const r = await fetch(url, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; AccidentCommandCenter/1.0; +https://accidentcommandcenter.com)',
        'Accept': 'text/html'
      },
    ...
    // Patch article URLs follow patch.com/<state>/<city>/<article-slug>
    ...
    while ((mm = articleRe.exec(html)) !== null) {
      ...
      // The slug itself describes the article; convert to a title-ish string
      const slug = path.split('/').pop();
      const title = slug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      items.push({
        title,
        link: `https://patch.com${path}`,
        desc: title, // we don't have description without fetching the article page
        pub: ''
      });
