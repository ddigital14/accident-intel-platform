...
async function ingestNYC(db, days = 3, limit = 100) {
  ...
  const url = `https://data.cityofnewyork.us/resource/h9gi-nx95.json?$where=crash_date>'${since}'&$limit=${limit}`;
  try {
    ...
    for (const x of rows) {
      ...
      const exists = await db('incidents').where('source_reference', ref).first();
...
const SPANISH_FEEDS = [
  'https://www.univision.com/feeds/rss/main',
  'https://www.telemundo.com/rss/noticias',
...
async function handler(req, res) {
  ...
  if (action === 'all') {
    const results = await Promise.all([ingestNYC(db, 3, 50), ingestNTSB(db, 14), ingestSpanish(db)]);
