...
// Phase 50: pull keyword library from centralized _pi_keywords (Spanish + workplace + premises + commercial added).
const { rotateQueries } = require('./_pi_keywords');
// Each cron tick picks 6 randomized PI phrases (rotation maximizes NewsAPI coverage under daily quota).
const SEARCH_QUERIES_DEFAULT = [
  'fatal crash',
...
const SEARCH_QUERIES = (() => {
  try { return rotateQueries(8); } catch (_) { return SEARCH_QUERIES_DEFAULT; }
})();
  ...
  try {
    ...
    for (const article of articles) {
      ...
      try {
        ...
        // Phase 39: regex pattern fallback — pull names the LLM missed
        try {
          const _src0 = (article?.title || '') + '\n' + (article?.description || '') + '\n' + (article?.content || '');
          const extra = extractExtraVictims(_src0, applyDenyList) || [];
          parsed.victims = parsed.victims || [];
          const have = new Set(parsed.victims.map(v => String(v.full_name || '').toLowerCase()));
          for (const ex of extra) {
            if (!ex || !ex.name) continue;
            if (have.has(ex.name.toLowerCase())) continue;
            parsed.victims.push({ full_name: ex.name, role: 'driver', _extracted_by: 'regex_pattern', _pattern_sources: ex.sources });
          }
        } catch (_) {}
        if (parsed.victims && parsed.victims.length) {
          for (const v of parsed.victims) {
            if (!v.full_name && !v.last_name) continue;
            const _candidate = v.full_name || `${v.first_name || ''} ${v.last_name || ''}`.trim();
            const _src = (article?.title || '') + '\n' + (article?.description || '') + '\n' + (article?.content || '');
            const _safe = applyDenyList(_candidate, _src);
            if (!_safe) { results.errors.push(`deny:${_candidate}`); continue; }
