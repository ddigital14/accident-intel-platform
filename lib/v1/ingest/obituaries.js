  ...
  try {
    ...
    for (const inc of fatalIncidents) {
      ...
      try {
        ...
        // Phase 39: regex pattern fallback against obit HTML, before iterating matches
        try {
          const obitText = (html || '').toString().replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').slice(0, 50000);
          const extra = extractExtraVictims(obitText, applyDenyList) || [];
          result.matches = result.matches || [];
          const have = new Set(result.matches.map(m => String(m.full_name || '').toLowerCase()));
          for (const ex of extra) {
            ...
            result.matches.push({
              full_name: ex.name,
              match_confidence: 65,
              match_reason: 'regex_pattern:' + (ex.sources || []).join(','),
              funeral_home: null,
              age: null,
              death_date: null,
              city_residence: inc.city || null
            });
          }
        } catch (_) {}
