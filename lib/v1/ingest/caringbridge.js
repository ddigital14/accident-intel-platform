...
const SEARCH_TERMS = [
  'caringbridge "car accident"',
  'caringbridge "car crash"',
  'caringbridge "hit by car"',
  'caringbridge "motorcycle accident"',
  'caringbridge "pedestrian struck"',
  'caringbridge "DUI"',
  'caringbridge "truck accident"',
  'caringbridge "rollover"'
];

async function braveSiteSearch(key, term) {
  // Brave's site: operator can be too restrictive; we add caringbridge as a keyword
  // term and then post-filter by URL. This catches more pages than site: alone.
  try {
    const r = await fetch(`https://api.search.brave.com/res/v1/web/search?q=${encodeURIComponent(term)}&count=10`, {
    ...
    return (j.web?.results || [])
      .filter(x => /caringbridge\.org/i.test(x.url || ''))
