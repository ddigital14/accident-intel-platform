...
// Phase 44A: VoyageAI semantic dedup. Run when 2 incidents have same
// occurred-date + city + similar address but PostGIS missed them (no geom or
// outside 1km). Embeds summaries and merges if cosine >= 0.85.
async function semanticDedupSameDayCity(db, results) {
  let voyage;
  try { voyage = require('../enrich/_voyage_router'); } catch (_) { return; }
  if (!voyage || typeof voyage.embed !== 'function') return;
  const sinceTs = new Date(Date.now() - 7 * 86400000); // last 7 days
  const SIM_THRESHOLD = 0.85;
  let groups;
  try {
    groups = await db.raw(`
      SELECT DATE(occurred_at) AS day, LOWER(COALESCE(city,'')) AS city, COUNT(*) AS c
      FROM incidents
      WHERE created_at > ?
  ...
  const rows = groups?.rows || groups || [];
  results.semantic_groups_checked = 0;
  results.semantic_merged = 0;
  for (const g of rows) {
    if (!g.day || !g.city) continue;
    results.semantic_groups_checked++;
    const incs = await db('incidents')
      .whereRaw('DATE(occurred_at) = ?', [g.day])
      .whereRaw('LOWER(COALESCE(city, \'\')) = ?', [g.city])
      .select('id', 'description', 'address', 'incident_type', 'severity', 'occurred_at')
      .orderBy('confidence_score', 'desc')
      .limit(8);
    if (incs.length < 2) continue;
    // Build a summary string for each
    const summaries = incs.map(i => [
      i.severity || '',
      i.incident_type || '',
      i.address || '',
      String(i.description || '').substring(0, 400)
    ].filter(Boolean).join(' | '));
    const vecs = await voyage.embedBatch(summaries, 'voyage-3', db);
    if (!vecs || vecs.length < 2) continue;
    // Compare top-confidence (idx 0) vs the rest
    const primary = incs[0], pv = vecs[0];
    if (!pv) continue;
    for (let j = 1; j < incs.length; j++) {
      const sec = incs[j], sv = vecs[j];
      if (!sv) continue;
      const sim = voyage.cosineSim(pv, sv);
      if (sim >= SIM_THRESHOLD) {
        try {
          // Reassign children, then delete secondary
          await db('source_reports').where('incident_id', sec.id).update({ incident_id: primary.id });
          await db('persons').where('incident_id', sec.id).update({ incident_id: primary.id });
          await db('vehicles').where('incident_id', sec.id).update({ incident_id: primary.id });
          await db('incidents').where('id', primary.id).update({
            description: (primary.description || '') + `\n[Semantic-merge sim=${sim.toFixed(3)} from ${sec.id}]`,
            updated_at: new Date()
          });
          await db('enrichment_logs').where('incident_id', sec.id).del();
          await db('activity_log').where('incident_id', sec.id).del();
          await db('incidents').where('id', sec.id).del();
          results.semantic_merged++;
        } catch (e) {
          // Soft-fail: tag as merged
          try {
            await db('incidents').where('id', sec.id).update({ status: 'merged', updated_at: new Date() });
          } catch (_) {}
        }
      }
    }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Phase 44A: semantic dedup pass (cross-day-city VoyageAI cosine match)
    try { await semanticDedupSameDayCity(db, results); } catch (e) { results.semantic_error = e.message; }
