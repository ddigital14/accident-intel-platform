...
async function runJob(jobName, parentReq) {
  ...
  try {
    ...
    // Phase 25: per-job safety timeout. Slow jobs (news-rss, scanner, court) get 25s,
    // everyone else 15s — keeps the 60s function envelope safe with 8 parallel jobs.
    const SLOW_JOBS = new Set(['news-rss','scanner','court','obituaries','people-search','state-crash','reddit','news','court-reverse-link','obit-backfill','identity-investigator','claude-reason','homegrown-rotation','pd-press','police-social','correlate']);
    const perJobTimeoutMs = SLOW_JOBS.has(jobName) ? 25000 : 15000;
    await Promise.race([
      fn(fakeReq, fakeRes),
