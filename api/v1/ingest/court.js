...
async function classifyAttorneyMentions(html, personName) {
  ...
  try {
    const resp = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: 'You are a legal-records analyst. Answer in strict JSON. Only mark has_attorney=true with confidence>70 if there is direct evidence of representation, NOT just general attorney advertising.' },
  ...
  try {
    ...
    for (const p of candidates) {
      try {
        ...
        const verdict = await classifyAttorneyMentions(html, name);
        results.checked++;
